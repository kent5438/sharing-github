...
process Otu_Preprocess {
  ...
  /opt/mothur_v1.44.0/mothur "#remove.lineage(fasta=${sampleid}.trimmed.unique.fa, count=${sampleid}.trimmed.count_table, taxonomy=${sampleid}.trimmed.unique.raw_taxonomy.wang.taxonomy, taxon='o__Chloroplast-f__Mitochondria-unknown')" > /dev/null # 輸出 XX.trimmed.unique.raw_taxonomy.wang.pick.taxonomy & XX.trimmed.unique.pick.fa & XX.trimmed.pick.count_table
...
//  memory 70.GB
  ...
  cp ${taxonomy} 97
  cd 97/
  /opt/mothur_v1.44.0/mothur "#dist.seqs(processors=${task.cpus}, fasta=${align}, countends=F, cutoff=0.03)" > /dev/null
  /opt/mothur_v1.44.0/mothur "#cluster(column=${sampleid}.trimmed.unique.dist, count=${count_table}, method=opti, cutoff=0.03)" > /dev/null
  /opt/mothur_v1.44.0/mothur "#make.shared(list=${sampleid}.trimmed.unique.opti_mcc.list, count=${count_table})" > /dev/null
  /opt/mothur_v1.44.0/mothur "#classify.otu(list=${sampleid}.trimmed.unique.opti_mcc.list, count=${count_table}, cutoff=80, label=0.03, taxonomy=$taxonomy)'" > /dev/null

  /opt/mothur_v1.44.0/mothur "#remove.lineage(constaxonomy=${sampleid}.trimmed.unique.opti_mcc.0.03.cons.taxonomy, shared=${sampleid}.trimmed.unique.opti_mcc.shared, taxon='unknown', label=0.03)"
  mv ${sampleid}.trimmed.unique.opti_mcc.0.03.cons.pick.cons.taxonomy ${sampleid}.trimmed.unique.opti_mcc.0.03.cons.taxonomy
  if [ -e "${sampleid}.trimmed.unique.opti_mcc.0.03.pick.shared" ]; then
    mv ${sampleid}.trimmed.unique.opti_mcc.0.03.pick.shared ${sampleid}.trimmed.unique.opti_mcc.shared
  fi
  
## OTUs ref ID, used for comparing with OTU sequence
  /opt/mothur_v1.44.0/mothur "#make.biom(shared=${sampleid}.trimmed.unique.opti_mcc.shared, constaxonomy=${sampleid}.trimmed.unique.opti_mcc.0.03.cons.taxonomy, matrixtype=dense)" > /dev/null
  mv ${sampleid}.trimmed.unique.opti_mcc.0.03.biom ${sampleid}.otu_table.otuID.biom


#  if [ ! -z "unknown_ids.txt" ]; then
...
#  fi
  ...
  /opt/mothur_v1.44.0/mothur "#get.oturep(column= ${sampleid}.trimmed.unique.dist, list= ${sampleid}.trimmed.unique.opti_mcc.list, count=${count_table}, fasta=${fa}, method=abundance)" > /dev/null

  ## Convert otuID.filter to taxID.filter
#  vsearch --usearch_global  ${sampleid}.trimmed.unique.opti_mcc.0.03.rep.fasta --db ${params.silva_otu_ref_fasta} --id 0.9 --blast6out ${sampleid}.results.blast6 --maxaccepts 1 --threads 32
#  blastn -query ${sampleid}.trimmed.unique.opti_mcc.0.03.rep.fasta -db ${params.silva_otu_ref_fasta} -num_threads 32 -outfmt 6 -max_target_seqs 1 -max_hsps 1 -out ${sampleid}.results.blast6
...
#  	${sampleid}.trimmed.unique.opti_mcc.0.03.rep.fasta,${sampleid}.results.blast6
...
process Tax4Fun {
  ...
  /opt/conda/bin/Rscript /export/EC1680U/perl/bin/16S/Tax4Fun/Tax4Fun.noPick.r
  """
}
