...
//  memory 70.GB
  ...
  /app_tools/mothur/mothur "#sub.sample(shared=${sampleid}.trimmed.unique.opti_mcc.shared, size=50000)" # WPM4.trimmed.unique.opti_mcc.0.03.subsample.shared
  ...
  /app_tools/mothur/mothur "#make.biom(shared=${sampleid}.trimmed.unique.opti_mcc.0.03.subsample.shared, constaxonomy=${sampleid}.trimmed.unique.opti_mcc.0.03.cons.taxonomy, matrixtype=dense)" > /dev/null
  mv ${sampleid}.trimmed.unique.opti_mcc.0.03.subsample.0.03.biom ${sampleid}.otu_table.otuID.biom
...
process Alpha_Diversity {
  ...
  if [ \$maximum_reads -ge "30000" ]; then
      multiple_rarefactions.py -i ${biom} --subsample_multinomial -m 1 -x \$maximum_reads -s 2500 -n 10 -o rarefaction/
  else
      multiple_rarefactions.py -i ${biom} --subsample_multinomial -m 1 -x \$maximum_reads -s 250 -n 10 -o rarefaction/
  fi
