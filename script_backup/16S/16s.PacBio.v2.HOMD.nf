...
process Otu_Preprocess {
  ...
  /opt/mothur_v1.44.0/mothur "#remove.lineage(fasta=${sampleid}.trimmed.unique.fa, count=${sampleid}.trimmed.count_table, taxonomy=${sampleid}.trimmed.unique.qiime.wang.taxonomy, taxon='o__Chloroplast-f__Mitochondria-unknown')" > /dev/null
  mv ${sampleid}.trimmed.unique.qiime.wang.pick.taxonomy ${sampleid}.trimmed.unique.silva.wang.taxonomy
...
#  	${sampleid}.trimmed.unique.opti_mcc.0.01.rep.fasta,${sampleid}.results.blast6
  python /export/EC1680U/perl/bin/16S/otuToTax.HOMD.py \
    ${sampleid}.otu_table.otuID.filter.dense.biom \
    ${sampleid}.otu_table.taxID.filter.dense.biom
...
process Upgma {
  ...
  #  weighted
  parallel_beta_diversity.py -i rarefaction/ -o weighted_unifrac/rare_dm/ -m weighted_unifrac -t ${params.silva_ref_tree} -O 20
  ...
  parallel_beta_diversity.py -i rarefaction/ -o unweighted_unifrac/rare_dm/ -m unweighted_unifrac -t ${params.silva_ref_tree} -O 20
