...
sed '1i #SampleID' ../SampleName_16S.txt > Map_file.txt
...
cat otu.table.txt | grep -v 'biom' | awk -F'\t' 'BEGIN{OFS="\t"} {$1=$NF;  print}' | awk -F'\t' 'BEGIN{OFS="\t"} {$NF=""; print}' > otu_table.forPCA.txt
...
$qiime summarize_taxa.py -i otu_table.biom -o taxa_plots -a  -L 1,2,3,4,5,6,7
$qiime_taxa plot_taxa_summary.py \
	-i otu_table_L1.txt,otu_table_L2.txt,otu_table_L3.txt,otu_table_L4.txt,otu_table_L5.txt,otu_table_L6.txt,otu_table_L7.txt \
...
ln -s /export/EC1680U/perl/bin/16S/taxaPlotParser.SILVA.pl .
sudo chown -R $username:$username taxa_plots
perl taxaPlotParser.SILVA.pl taxa_plots/raw_data taxa_plots
...
ln -s /export/EC1680U/perl/bin/16S/alphaBetaDiversity.SILVA.pl .
BETA_DIR=meta16S_OUTPUT/AlphaBetaDiversity
mkdir $BETA_DIR
# need to modified alphaBetaDiversity.SILVA.pl content for docker distribution
perl alphaBetaDiversity.SILVA.pl otu_table.biom $BETA_DIR
