...
process Join_Fastq {
  ...
  script:
  """
  /export/EC1680U/software/anaconda2/bin/pear -n 50 -s 2 -u 1 -p 0.05 -v 15 -j 32 -f  ${trim_r1} -r ${trim_r2} -o ${sampleid}.pear 2>&1 | tee pear.log
  mv ${sampleid}.pear.assembled.fastq ${sampleid}.extendedFrags.fastq  
  """
}
...
process Statistic_Analysis_part2 {
  ...
  import glob
  import os
  fns = sorted(glob.glob("*.statistic_sample.txt"))
...
process Taxa_Summary {
  ...
  perl /export/EC1680U/perl/bin/16S/taxaPlotParser_forCompassPipe.pl taxa_summary_plots/raw_data/ taxa_summary_plots/
  """
}
