...
if [ ! -e "$DIR/$MSFOLDER/${MSID}-16S.*.clean.fastq" ]; then
for i in $(ls *_001.clean.fastq.gz); 
do 
#pigz -cd -p 32 ${MSID}-16S_S*_R1_001.clean.fastq.gz > "$DIR/$MSFOLDER/${MSID}-16S.R1.clean.fastq"
#pigz -cd -p 32 ${MSID}-16S_S*_R2_001.clean.fastq.gz > "$DIR/$MSFOLDER/${MSID}-16S.R2.clean.fastq"
pigz -cd -p 32 ${MSID}_S*_R1_001.clean.fastq.gz > "$DIR/$MSFOLDER/${MSID}-16S.R1.clean.fastq"
pigz -cd -p 32 ${MSID}_S*_R2_001.clean.fastq.gz > "$DIR/$MSFOLDER/${MSID}-16S.R2.clean.fastq"
done | sort | uniq
fi
...
perl ../demultiplexFASTQ_byBarcodes_4mer.pl ${i}.R1.clean.fastq ${i}.R2.clean.fastq ${MSID}-16S-barcode_1.txt _demultiplexed_${i} _baseSpace_${i} 2>&1 | tee -a demultiplexFASTQ-${i}.pl.log
done
