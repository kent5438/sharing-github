...
mothur "#remove.lineage(fasta=trimmed.unique.fa, name=trimmed.names, group=group.txt, taxonomy=trimmed.unique.gg.wang.taxonomy, taxon=Chloroplast-Mitochondria-unknown-mitochondria-chloroplast)" > /dev/null
mv trimmed.unique.gg.wang.pick.taxonomy trimmed.unique.gg.wang.taxonomy
mv trimmed.unique.pick.fa trimmed.unique.fa
mv trimmed.pick.names trimmed.names
mv group.pick.txt group.txt
...
filter_otus_from_otu_table.py -i otu_table.otuID.biom -o otu_table.otuID.filter.biom --min_count_fraction 0.00005 # ref: https://goo.gl/EMwR9N
...
#/export/EC1680U/perl/bin/16S/otuID_to_reftax.pl	# input: otu_table.greengenesID.biom|txt, output: otu_table.taxID.filter.biom|txt
...
rm -rf D.1_translated && translate_Qiime_to_Krnoa.pl D.1_taxa_summary/otu_table.greengenesID_L6.txt D.1_translated
echo '        run Krona'
...
echo '    E.3 LEfSe'
rm -rf E.3_LEfSe && run_LEfSe.pl D.1_taxa_summary/otu_table.greengenesID_L6.txt $mapping_file "$lefse_group" E.3_LEfSe
...
plot_3A.pl D.1_taxa_summary/otu_table.greengenesID_L7.txt $mapping_file 'Species' 'F_3A'
