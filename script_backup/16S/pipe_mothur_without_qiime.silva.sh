...
mothur "#classify.seqs(processors=$thread, fasta=trimmed.unique.fa, name=trimmed.names, template=$silva_otu_ref_fasta, taxonomy=$silva_otu_ref_taxonomy, cutoff=$classify_cutoff, method=wang)" > /dev/null
mothur "#remove.lineage(fasta=trimmed.unique.fa, count=trimmed.count_table, taxonomy=trimmed.unique.raw_taxonomy.wang.taxonomy, taxon=Chloroplast-Mitochondria-unknown-mitochondria-chloroplast)" > /dev/null
mv trimmed.unique.raw_taxonomy.wang.pick.taxonomy trimmed.unique.raw_taxonomy.wang.taxonomy
mv trimmed.unique.pick.fa trimmed.unique.fa
mv trimmed.pick.count_table trimmed.count_table

mothur "#align.seqs(processors=$thread, candidate=trimmed.unique.fa, template=$silva_otu_ref_aligned_fasta)" > /dev/null
...
#mothur "#classify.otu(list=trimmed.unique.opti_mcc.list, count=../trimmed.count_table, cutoff=$classify_cutoff, taxonomy=../trimmed.unique.silva.wang.taxonomy)" > /dev/null
#awk -F "\t" '{if(substr($3,1,7)!="unknown"){print}}' trimmed.unique.opti_mcc.0.03.cons.taxonomy > trim_unknown.txt
#mothur "#make.biom(shared=trimmed.unique.opti_mcc.shared, constaxonomy=trim_unknown.txt, matrixtype=dense, reftaxonomy=$silva_otu_ref_taxonomy, picrust=$mothur_otu_ref_map)" > /dev/null
...
mothur "#make.shared(list=trimmed.unique.opti_mcc.list, count=../trimmed.count_table)" > /dev/null
mothur "#classify.otu(list=trimmed.unique.opti_mcc.list, count=../trimmed.count_table, cutoff=80, label=0.03, taxonomy=../trimmed.unique.raw_taxonomy.wang.taxonomy)" > /dev/null
awk -F "\t" '{if(substr($3,1,7)!="unknown"){print}}' trimmed.unique.opti_mcc.0.03.cons.taxonomy > trim_unknown.txt
...
grep 'unknown;' trimmed.unique.opti_mcc.0.03.cons.taxonomy | cut -f1 > unknown_ids.txt
filter_otus_from_otu_table.py -i otu_table.otuID.biom -e unknown_ids.txt -o otu_table.otuID.filter.biom --min_count_fraction 0.00005 # ref: https://goo.gl/EMwR9N
biom convert -i otu_table.otuID.filter.biom -o otu_table.otuID.filter.txt --to-tsv --header-key taxonomy --table-type="OTU table"
...
#mothur "#get.oturep(column=trimmed.unique.dist, list=trimmed.unique.opti_mcc.list, name=../trimmed.names, fasta=../trimmed.unique.fa)" > /dev/null
...
perl /export/EC1680U/perl/bin/16S/biomProc.pl sparseToDense \
	otu_table.otuID.filter.biom \
	otu_table.otuID.filter.dense.biom

perl /export/EC1680U/perl/bin/16S/biomProc.pl idChangeUsingFasta \
	otu_table.otuID.filter.dense.biom \
	otu_table.taxID.filter.dense.biom \
	trimmed.unique.opti_mcc.0.03.rep.fasta,results.blast6

perl /export/EC1680U/perl/bin/16S/biomProc.pl uniqById \
	otu_table.taxID.filter.dense.biom \
	otu_table.taxID.filter.dense.dedup.biom
########################

biom convert -i otu_table.taxID.filter.dense.dedup.biom -o otu_table.taxID.filter.dense.dedup.txt \
...
plot_phylogenetic_tree.pl $workPath/$otu_tax_dedup_table_97 $silva_otu_ref_aligned_fasta
cd $workPath
...
cp $otu_filter_table_97 $otu_filter_biom_97 report_html/data/OTU_Statistic/
cp $otu_tax_dedup_table_97 $otu_tax_dedup_biom_97 report_html/data/OTU_Statistic/
