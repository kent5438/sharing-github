...
if($pwd =~ /ITS/){
	...
	if (!$tree) {
		$tree = '/export/EC1680U/DataBase/16S/SILVA_132_release/trees/99/99_otus.tre';
	}
	system("rsync -avq $tree .");
}
...
# Alpha diversity
if (fork() == 0) {
  my $cmd = $qiime_docker.' alpha_diversity.py -i '.$input.' -o '.$output.'alpha_'.$outputFileName.' -m chao1,enspie,fisher_alpha,goods_coverage,observed_species,shannon,simpson_reciprocal -t 99_otus.tre';
  if ($pwd =~ /ITS/) {
...
# Beta diversity
if (fork() == 0) {
  my $cmd = $qiime_docker .' beta_diversity.py -i '.$input.' -o '.$output.' -m weighted_unifrac,unweighted_unifrac -t 99_otus.tre';
  if ($pwd =~ /ITS/) {
