...
#Clean_path="/export/EC2480U/Miseq/$MSID"
#ln -s $Clean_path .
...
#else
#for i in $(ls *.fastq.gz | awk -F'_L001' '{print $1}' | awk -F'_S' '{print $1}') ; 
#do 
#echo "pigz -cd -p 32 ${i}_L001_R1_001.clean.fastq.gz > $i.R1.clean.fastq" ; 
#echo -e "pigz -cd -p 32 ${i}_L001_R2_001.clean.fastq.gz > $i.R2.clean.fastq" ; 
#pigz -cd -p 32 ${i}_S*_L001_R1_001.clean.fastq.gz > "$DIR/$MSFOLDER/$i.R1.clean.fastq"
#pigz -cd -p 32 ${i}_S*_L001_R2_001.clean.fastq.gz > "$DIR/$MSFOLDER/$i.R2.clean.fastq"
#done | sort | uniq
...
#cd $DIR/$MSFOLDER
#mkdir 0_cleanFastq
#ln -s /export/EC1680U/kentchen/16S/$MSFOLDER/*.fastq 0_cleanFastq 
#cd 0_cleanFastq
#rm -rf Undetermined*
...
echo "*** Start program of 16S analysis pipeline..."
perl SR_16S_pipe.pl -p V3FV4R_primer_V3V4.alignseq -s SampleName_16S.txt -r Y -o Y -i Y 2>&1 | tee -a MiSeq16Sanalysis_pipeline.v3v4.MS16121.log
...
ssh -t -t qiime@192.168.0.14<<EOF
...
sh qiime_16S_statistics_V3V4.kentchen.sh $MSID 2>&1 | tee qiime_process.log
