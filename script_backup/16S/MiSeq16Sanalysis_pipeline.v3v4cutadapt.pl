...
for ($i=0;$i< $sCount ;$i++)
{
	...
	else 
	{
		...
		print "pear -n 50 -s 2 -u 1 -p 0.05 -v 15 -j 24 -o $sampleIDs[$i] -f ../$folder[0]/$sampleIDs[$i].R1.$fastqSuffix[0] -r ../$folder[0]/$sampleIDs[$i].R2.$fastqSuffix[0] &> $sampleIDs[$i].pear.log\n";
	  	    `pear -n 50 -s 2 -u 1 -p 0.05 -v 15 -j 24 -o $sampleIDs[$i] -f ../$folder[0]/$sampleIDs[$i].R1.$fastqSuffix[0] -r ../$folder[0]/$sampleIDs[$i].R2.$fastqSuffix[0] &> $sampleIDs[$i].pear.log`;
	...
	else 
	{
		...
		if ($trimOption== 1)
		{
			...
			if ($primerSet =~ /V3V4/)
			{
				`/export/EC1680U/software/anaconda2/bin/cutadapt -g AGAGTTTGATCMTGGCTCAG -a GTATTACCGCGGCKGCTG --discard-untrimmed -o $sampleIDs[$i].$fastqSuffix[2] ../$folder[1]/$sampleIDs[$i].$fastqSuffix[1] &> $sampleIDs[$i].assembledTrimPrimers.log`;
	...
	else 
	{
		...
		if ($primerSet =~ /V3V4/)
		{
			print "java -jar /export/EC1680U/software/trimmomatic-0.33.jar SE -threads 24 -phred33 -trimlog temp_$sampleIDs[$i].log ../$folder[2]/$sampleIDs[$i].$fastqSuffix[2] $sampleIDs[$i].$fastqSuffix[3] SLIDINGWINDOW:10:10 MINLEN:300\n";		
		  	    `java -jar /export/EC1680U/software/trimmomatic-0.33.jar SE -threads 24 -phred33 -trimlog temp_$sampleIDs[$i].log ../$folder[2]/$sampleIDs[$i].$fastqSuffix[2] $sampleIDs[$i].$fastqSuffix[3] SLIDINGWINDOW:10:10 MINLEN:100`;
		}
		else
		{
...
#		`/export/EC1680U/software/fastaQual2fastq.pl ../$folder[4]/$sampleIDs[$i].effective.fasta ../$folder[4]/$sampleIDs[$i].effective.qual 33 1000`;
	...
	{
	print getCurrentDateTime()."\t";
	print "mothur \"#classify.seqs(processors=30,fasta=$sampleIDDash[$i].unique.fasta,name=$sampleIDDash[$i].names,reference=$refFastaDB[1],taxonomy=$refTaxonomy[1], cutoff=60, method=wang, probs=f)\"\n";
	`/export/EC1680U/software/anaconda2/bin/mothur "#classify.seqs(processors=30,fasta=$sampleIDDash[$i].unique.fasta,name=$sampleIDDash[$i].names,reference=$refFastaDB[1],taxonomy=$refTaxonomy[1], cutoff=60, method=wang, probs=f)"`;
...
if ($removeLineageTag==1) 
{
	...
	`/export/EC1680U/software/anaconda2/bin/mothur "#remove.lineage(taxonomy=$sampleIDs[$i].unique.gg.wang.taxonomy,taxon=Chloroplast-Mitochondria-unknown-mitochondria-chloroplast,fasta=$sampleIDs[$i].unique.fasta,name=$sampleIDs[$i].names, group=$sampleIDs[$i].groups)"`;
	...
	print getCurrentDateTime()."\t";
	print "mothur \"#classify.otu(list=$sampleIDs[$i].unique.filter.an.list,count=$sampleIDs[$i].count_table, taxonomy=$sampleIDs[$i].unique.gg.wang.taxonomy, cutoff=60, probs=f)\"\n";
	`/export/EC1680U/software/anaconda2/bin/mothur "#classify.otu(list=$sampleIDs[$i].unique.filter.an.list,count=$sampleIDs[$i].count_table, taxonomy=$sampleIDs[$i].unique.gg.wang.taxonomy, cutoff=60, probs=f)"`;
  ...
  print "mothur \"#make.biom(shared=$sampleIDs[$i].unique.filter.an.shared,constaxonomy=$sampleIDs[$i].unique.filter.an.unique.cons.taxonomy, matrixtype=dense, reftaxonomy=$refTaxonomy[1], picrust=$refotuMapTable[1])\"\n";
	...
	print getCurrentDateTime()."\t";
	print "mothur \"#get.oturep(column=$sampleIDs[$i].unique.filter.dist, list=$sampleIDs[$i].unique.filter.an.list, name=$sampleIDs[$i].names, fasta=$sampleIDs[$i].unique.filter.fasta)\"\n";
	`/export/EC1680U/software/anaconda2/bin/mothur "#get.oturep(column=$sampleIDs[$i].unique.filter.dist, list=$sampleIDs[$i].unique.filter.an.list, name=$sampleIDs[$i].names, fasta=$sampleIDs[$i].unique.filter.fasta)"`;
	#Output File Names: 
