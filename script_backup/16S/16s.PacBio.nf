...
process Otu_Similarity97 {
  ...
  output:
  set val(group), val(sampleid), file("97/${sampleid}.trimmed.unique.opti_mcc.list"), file("97/${sampleid}.trimmed.unique.opti_mcc.0.cons.taxonomy"),  file("97/${sampleid}.otu_table.taxID.filter.dense.dedup.biom"), file("97/${sampleid}.otu_table.taxID.filter.dense.dedup.txt"), file('97/*') into otu_output_similarity97
  ...
  cp ${taxonomy} 97
  cd 97/
  /app_tools/mothur/mothur "#dist.seqs(processors=${task.cpus}, fasta=${align}, countends=F, cutoff=0.01)" > /dev/null
  /app_tools/mothur/mothur "#cluster(column=${sampleid}.trimmed.unique.dist, count=${count_table}, method=opti, cutoff=0.01)" > /dev/null
  /app_tools/mothur/mothur "#make.shared(list=${sampleid}.trimmed.unique.opti_mcc.list, count=${count_table}, label=0.01)" > /dev/null
  /app_tools/mothur/mothur "#classify.otu(list=${sampleid}.trimmed.unique.opti_mcc.list, count=$count_table, taxonomy=$taxonomy, label=0.01)'" > /dev/null
  #grep 'unknown;' ${sampleid}.trimmed.unique.opti_mcc.0.cons.taxonomy | cut -f1 > unknown_ids.txt
  ## OTUs ref ID, used for comparing with OTU sequence
  /app_tools/mothur/mothur "#make.biom(shared=${sampleid}.trimmed.unique.opti_mcc.shared, constaxonomy=${sampleid}.trimmed.unique.opti_mcc.0.cons.taxonomy, matrixtype=dense)" > /dev/null
  mv ${sampleid}.trimmed.unique.opti_mcc.0.biom ${sampleid}.otu_table.otuID.biom
  ...
  /app_tools/mothur/mothur "#get.oturep(column= ${sampleid}.trimmed.unique.dist, list= ${sampleid}.trimmed.unique.opti_mcc.list, count=${count_table}, fasta=${fa}, method=abundance, label=0.01)" > /dev/null

  ## Convert otuID.filter to taxID.filter
#  vsearch --usearch_global  ${sampleid}.trimmed.unique.opti_mcc.0.rep.fasta --db ${params.silva_otu_ref_fasta} --id 0.1 --iddef 1 --blast6out results.blast6 --maxaccepts 1 --threads 32
  blastn -query ${sampleid}.trimmed.unique.opti_mcc.0.rep.fasta -db ${params.silva_otu_ref_fasta} -num_threads 32 -outfmt 6 -max_target_seqs 1 -max_hsps 1 -out ${sampleid}.results.blast6
  ...
  perl /export/EC1680U/perl/bin/16S/biomProc.pl idChangeUsingFasta \
  	...
  	${sampleid}.trimmed.unique.opti_mcc.0.rep.fasta,${sampleid}.results.blast6
