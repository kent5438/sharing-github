...
sub init {
  our %cfg = init_cfg ("PB_report-16s.cfg");
...
sub report {
  ...
  # CleanPE statistics
  $content = $content."<li><a href=html/multiqc_report.html target=display>ReadQC</a></li>";
  # barcodes statistics
  ...
  if ($samplesize >= 3) {
	...
	#OTUs_heatmap
#    $content = $content."<li><a href=OTUs_heatmap/otu_table.html target='display'>OTU heatmap</a></li>";
    $content = $content.'<li><a href=OTUs_heatmap/OTUs_heatmap.pdf target=display>OTU heatmap (genus)</a></li>';
    $content = $content.'<li><a href=OTUs_heatmap/Sample_correlation.pdf target=display>Sample Correlation</a></li>';
	#PCA
	$content = $content."<li><a href=files/PCA.pdf target=display>PCA plot</a></li>";
#	$content = $content. "<li><a href=PCoA/beta_div_coords_2D_PCoA_plots.html target='display'>PCoA 2D plot</a></li>";
  }
...
sub inithtml {
  ...
  $cmd = "cp -r /export/EC1680U/kentchen/Template/PB_16S/css $output";
  push (@cmds, $cmd);
  $cmd = "cp -r /export/EC1680U/kentchen/Template/PB_16S/images/* $output/images";
  push (@cmds, $cmd);
  $cmd = "cp -r /export/EC1680U/kentchen/Template/PB_16S/DataAnalysisWorkflow.html $output/html/";
  push (@cmds, $cmd);
  $cmd = "cp -r /export/EC1680U/kentchen/Template/Help/PB-16S_Help.pdf $output/files/help.pdf";
  push (@cmds, $cmd);
