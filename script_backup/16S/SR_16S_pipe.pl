...
$refAlignDB[0]="/export/EC1680U/metagenomics/16Sdb/SILVA/silva.gold.align"; #  5,181
$refAlignDB[1]="/export/EC1680U/metagenomics/16Sdb/MothurUsage/greenGenes/gg_13_8_99.refalign"; # 203,452 
if (!-e $refAlignDB[1]){die "$refAlignDB[1] not found!!\n\n"}

$refFastaDB[1]="/export/EC1680U/metagenomics/16Sdb/MothurUsage/greenGenes/gg_13_8_99.fasta";
$refFastaDB[2]="/export/EC1680U/metagenomics/16Sdb/MothurUsage/gg_13_8_99.fasta"; #
if (!-e $refFastaDB[1]){die "$refFastaDB[1] not found!!\n\n"}

$refTaxonomy[1]="/export/EC1680U/metagenomics/16Sdb/MothurUsage/greenGenes/gg_13_8_99.gg.tax"; # 4	Root;k__Archaea;p__[Parvarchaeota];c__[Parvarchaea];o__YLA114;
...
for ($i=0;$i< $sCount ;$i++)
{
	...
	if (-e "./$folder[0]/$sampleIDs[$i].$fastqSuffix[0]")
	{
		$cleanPEgzTag=0;
		$fileExistCK=0;
	}
	elsif (-e "./$folder[0]/$sampleIDs[$i].$fastqSuffix[0].gz")
	{
		$cleanPEgzTag=1;
		$fileExistCK=0;
	}
	...
	{
		print "./$folder[0]/$sampleIDs[$i].$fastqSuffix[0](or .gz) not found!!\n";
		$fileExistCK=1;
		exit 1;
	}
	...
	else 
	{
		...
		if ($cleanPEgzTag==1)
		{
			`gzip -dc ./$folder[0]/$sampleIDs[$i].$fastqSuffix[0].gz > ./$folder[0]/$sampleIDs[$i].$fastqSuffix[0]`;
			`gzip -dc ./$folder[0]/$sampleIDs[$i].$fastqSuffix[0].gz > ./$folder[0]/$sampleIDs[$i].$fastqSuffix[0]`;
		}
		chdir ("./$folder[1]");
...
#		print "pear -m 500 -n 50 -s 2 -u 1 -p 0.05 -v 15 -j 24 -o $sampleIDs[$i] -f ../$folder[0]/$sampleIDs[$i].$fastqSuffix[0] -r ../$folder[0]/$sampleIDs[$i].$fastqSuffix[0] &> $sampleIDs[$i].pear.log\n";
#	  	    `/export/EC1680U/software/pear-0.9.10-bin-64/pear -m 500 -n 50 -s 2 -u 1 -p 0.05 -v 15 -j 24 -o $sampleIDs[$i] -f ../$folder[0]/$sampleIDs[$i].$fastqSuffix[0] -r ../$folder[0]/$sampleIDs[$i].$fastqSuffix[0] &> $sampleIDs[$i].pear.log`;

		chdir ("..");
		if ($cleanPEgzTag==1)
		{
			# if gz file exists, then delete the uncompressed fastq file
			if (-e "./$folder[0]/$sampleIDs[$i].$fastqSuffix[0].gz")
			{
				unlink ("./$folder[0]/$sampleIDs[$i].$fastqSuffix[0]");
			}
			if (-e "./$folder[0]/$sampleIDs[$i].$fastqSuffix[0].gz")
			{
				unlink ("./$folder[0]/$sampleIDs[$i].$fastqSuffix[0]");
	...
	else 
	{
		...
		print "mothur \"#fastq.info(format=illumina1.8+,qfile=T,fastq=$sampleIDs[$i].$fastqSuffix[3])\"\n";
					`/export/EC1680U/software/mothur/mothur "#fastq.info(format=illumina1.8+,qfile=T,fastq=$sampleIDs[$i].$fastqSuffix[3])"`;
	...
	if (!-e "./$folder[3]/$sampleIDs[$i].qualified.uchime.accnos" || !-e "./$folder[3]/$sampleIDs[$i].qualified.uchime.chimeras")
	{
		...
		print "\nPerform Mothur 'chimera.uchime' step automatically... \n\n";
			...
			`/export/EC1680U/software/mothur/mothur "#chimera.uchime(processors=30,fasta=$sampleIDDash[$i].$fastaSuffix[3], reference=$ref2ChimeraUchime)"`;
			`/export/EC1680U/software/mothur/mothur "#remove.seqs(accnos=$sampleIDs[$i].qualified.uchime.accnos,fasta=$sampleIDs[$i].$fastaSuffix[3],qfile=$sampleIDs[$i].$qualSuffix[3])"`;
#			`mv $sampleIDs[$i].qualified.pick.fasta ../$folder[4]/$sampleIDs[$i].$fastaSuffix[4]`;
			`mv $sampleIDs[$i].qualified.fasta ../$folder[4]/$sampleIDs[$i].$fastaSuffix[4]`;
#			`mv $sampleIDs[$i].qualified.pick.qual ../$folder[4]/$sampleIDs[$i].$qualSuffix[4]`;
			`mv $sampleIDs[$i].qualified.qual ../$folder[4]/$sampleIDs[$i].$qualSuffix[4]`;
...
if ($readStatTag eq "Y")
{
	...
	for ($f=0;$f< 5 ;$f++)
	{
		...
		for ($i=0;$i< $sCount ;$i++)
		{
			...
			{
				if ($cleanPEgzTag==1)
				{($total,$totalLen,$avgLen)=statFastq("$folder[0]/$sampleIDs[$i].$fastqSuffix[0].gz",$cleanPEgzTag);}
				else
				{($total,$totalLen,$avgLen)=statFastq("$folder[0]/$sampleIDs[$i].$fastqSuffix[0]",$cleanPEgzTag);}
				$cleanReadCount{$sampleIDs[$i]}=$total;
				$cleanBaseCount{$sampleIDs[$i]}=$totalLen;
				
				print "$sampleIDs[$i].$fastqSuffix[0]: total $cleanReadCount{$sampleIDs[$i]} reads, $cleanBaseCount{$sampleIDs[$i]} bases, average $avgLen bp in length\n";
			}
			if ($f==1) #joined fastq
			{
				if ($joinedGzTag==1)
...
for ($i=0;$i< $sCount ;$i++)
{
	...
	`/export/EC1680U/software/mothur/mothur "#make.group(fasta=$sampleIDDash[$i].$fastaSuffix[5], groups=$sampleIDDash[$i])"`;
	#Output File Names: 
	#T635-1TEST.groups
	print getCurrentDateTime()."\t";
	print "mothur \"#unique.seqs(fasta=$sampleIDs[$i].$fastaSuffix[5])\"\n";
	`/export/EC1680U/software/mothur/mothur "#unique.seqs(fasta=$sampleIDs[$i].$fastaSuffix[5])"`;
	#Output File Names: 
	#T635-1TEST.names
	#T635-1TEST.unique.fasta
	print getCurrentDateTime()."\t";
	print "mothur \"#count.seqs(processors=4, name=$sampleIDs[$i].names, group=$sampleIDs[$i].groups)\"\n";
	`/export/EC1680U/software/mothur/mothur "#count.seqs(processors=4, name=$sampleIDs[$i].names, group=$sampleIDs[$i].groups)"`;
	#Output File Names: 
	#T635-1TEST.count_table
	
	if (!-e "$sampleIDs[$i].unique.gg.wang.taxonomy")
	{
	print getCurrentDateTime()."\t";
	print "mothur \"#classify.seqs(processors=30,fasta=$sampleIDDash[$i].unique.fasta,name=$sampleIDDash[$i].names,reference=$refFastaDB[1],taxonomy=$refTaxonomy[1], cutoff=60, method=wang)\"\n";
	`/export/EC1680U/software/mothur/mothur "#classify.seqs(processors=30,fasta=$sampleIDDash[$i].unique.fasta,name=$sampleIDDash[$i].names,reference=$refFastaDB[1],taxonomy=$refTaxonomy[1], cutoff=60, method=wang)"`;
	...
	#
	#Output File Names: 
	#T635-1TEST.unique.gg.wang.taxonomy
	#T635-1TEST.unique.gg.wang.tax.summary
	#T635-1TEST.unique.gg.wang.flip.accnos
	}
...
{
	print getCurrentDateTime()."\t";
	print "mothur \"#remove.lineage(taxonomy=$sampleIDs[$i].unique.gg.wang.taxonomy,taxon=unknown,fasta=$sampleIDs[$i].unique.fasta,name=$sampleIDs[$i].names, group=$sampleIDs[$i].groups)\"\n";
	`/export/EC1680U/software/mothur/mothur "#remove.lineage(taxonomy=$sampleIDs[$i].unique.gg.wang.taxonomy,taxon=unknown,fasta=$sampleIDs[$i].unique.fasta,name=$sampleIDs[$i].names, group=$sampleIDs[$i].groups)"`;

	#Output File Names: 
	#T635-1TEST.unique.gg.wang.pick.taxonomy
	...
	`/export/EC1680U/software/mothur/mothur "#align.seqs(processors=30, flip=F, save=T, candidate=$sampleIDDash[$i].unique.fasta, template=$refAlignDB[1])"`;
	...
	`/export/EC1680U/software/mothur/mothur "#summary.seqs(processors=4,fasta=$sampleIDs[$i].unique.align, count=$sampleIDs[$i].count_table)"`;
	#Output File Names: 
	#T635-1TEST.unique.summary

	print getCurrentDateTime()."\t";
	print "mothur \"#filter.seqs(processors=10,fasta=$sampleIDDash[$i].unique.align, vertical=T)\"\n";
	`/export/EC1680U/software/mothur/mothur "#filter.seqs(processors=10,fasta=$sampleIDDash[$i].unique.align, vertical=T)"`;
	...
	`/export/EC1680U/software/mothur/mothur "#dist.seqs(processors=30,fasta=$sampleIDs[$i].unique.filter.fasta,calc=onegap,countends=F,cutoff=0.03)"`;
	#Output File Names: 
	...
	{
	print getCurrentDateTime()."\t";
	print "mothur \"#cluster(column=$sampleIDs[$i].unique.filter.dist, name=$sampleIDs[$i].names, cutoff=0.03, hard=t, method=average)\"\n";
	`/export/EC1680U/software/mothur/mothur "#cluster(column=$sampleIDs[$i].unique.filter.dist, name=$sampleIDs[$i].names, cutoff=0.03, hard=t, method=average)"`; #By default cluster() uses the average neighbor algorithm;
	...
	`/export/EC1680U/software/mothur/mothur "#make.shared(list=$sampleIDs[$i].unique.filter.an.list, group=$sampleIDs[$i].groups)"`;
	#Output File Names: 
	#T635-1TEST.unique.filter.an.shared
	#T635-1TEST.unique.filter.an.T635-1TEST.rabund

	print getCurrentDateTime()."\t";
	print "mothur \"#classify.otu(list=$sampleIDs[$i].unique.filter.an.list,count=$sampleIDs[$i].count_table, taxonomy=$sampleIDs[$i].unique.gg.wang.taxonomy, cutoff=60)\"\n";
	`/export/EC1680U/software/mothur/mothur "#classify.otu(list=$sampleIDs[$i].unique.filter.an.list,count=$sampleIDs[$i].count_table, taxonomy=$sampleIDs[$i].unique.gg.wang.taxonomy, cutoff=60)"`;
	...
	print getCurrentDateTime()."\t";
  print "mothur \"#make.biom(shared=$sampleIDs[$i].unique.filter.an.shared,constaxonomy=$sampleIDs[$i].unique.filter.an.unique.cons.taxonomy, matrixtype=dense, reftaxonomy=$refTaxonomy[1], picrust=$refotuMapTable[1])\"\n";
	`/export/EC1680U/software/mothur/mothur "#make.biom(shared=$sampleIDs[$i].unique.filter.an.shared,constaxonomy=$sampleIDs[$i].unique.filter.an.unique.cons.taxonomy, matrixtype=dense, reftaxonomy=$refTaxonomy[1], picrust=$refotuMapTable[1])"`;
	#Output File Names: 
	...
	`/export/EC1680U/software/mothur/mothur "#get.oturep(fasta=$sampleIDs[$i].unique.fasta, list=$sampleIDs[$i].unique.filter.an.list, name=$sampleIDs[$i].names, method=abundance)"`;
	#Output File Names: 
