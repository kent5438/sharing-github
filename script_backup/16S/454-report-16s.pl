#!/usr/bin/perl -w
# Author: Daniel Liu
# perl report-16s.pl.pl --sample=list.txt
# Date: 2016.11.25
...
sub init {
  our %cfg = init_cfg ("report-16s.cfg");
...
#  $content = $content."<li><a href=html/barcode.html target=display>Barcode Stats</a></li>";
  ...
  #rarefaction_plots
  $content = $content."<li><a href=rare_alpha_plot/rarefaction_plots.html target=_blank>rarefaction plots</a></li>";
    
  #taxa_plots bar_charts
  $content = $content."<li><a href=taxa_plots/bar_charts.html target=_blank>taxa_plots bar plots</a></li>";
  
  if ($samplesize >= 3) {
    #taxa_plots area_chartt
    $content = $content."<li><a href=taxa_plots/area_charts.html target=_blank>taxa_plots area plots</a></li>";
	#OTUs_heatmap
    $content = $content."<li><a href=OTUs_heatmap/otu_table.html target=_blank>OTU heatmap</a></li>";
	#PCA
	$content = $content."<li><a href=files/PCA.pdf target=_blank>PCA plot</a></li>";
  }
...
sub inithtml {
  ...
  $cmd = "cp -r /export/EC1680U/kentchen/Template/16S/css $output";
  push (@cmds, $cmd);
  $cmd = "cp -r /export/EC1680U/kentchen/Template/16S/images/* $output/images";
  push (@cmds, $cmd);
  $cmd = "cp -r /export/EC1680U/kentchen/Template/16S/DataAnalysisWorkflow.html $output/html/";
  push (@cmds, $cmd);
  $cmd = "cp -r /export/EC1680U/kentchen/Template/16S/help.pdf $output/files/";
  push (@cmds, $cmd);
#  $cmd = "cp -r `ls meta16S_OUTPUT/* | grep -v PCA.pdf` $output";
#  push (@cmds, $cmd);
  $cmd = "cp -r meta16S_OUTPUT/* $output";
  push (@cmds, $cmd);
