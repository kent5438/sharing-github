...
foreach $readLocation (@readLocationArray)
{
	...
	if ($bNum==2)
	{
		...
		if (defined($b2Sample{$R1headSeqF}{$R2headSeqR}))
		{
			...
			$readSeqTrimBC{$readLocation}[1]=	substr($readSeq{$readLocation}[1],0);
			$readSeqTrimBC{$readLocation}[2]=	substr($readSeq{$readLocation}[2],0);
			
			$readQualTrimBC{$readLocation}[1]=	substr($readQul{$readLocation}[1],0);
			$readQualTrimBC{$readLocation}[2]=	substr($readQul{$readLocation}[2],0);
		...
		elsif (defined($b2Sample{$R2headSeqF}{$R1headSeqR}))
		{
			...
			$readSeqTrimBC{$readLocation}[1]=	substr($readSeq{$readLocation}[2],0);
			$readSeqTrimBC{$readLocation}[2]=	substr($readSeq{$readLocation}[1],0);
			
			$readQualTrimBC{$readLocation}[1]=	substr($readQul{$readLocation}[2],0);
			$readQualTrimBC{$readLocation}[2]=	substr($readQul{$readLocation}[1],0);
