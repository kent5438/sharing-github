...
list = Channel
  ...
  .map { line ->
    ...
    if (id=~/#SampleID/) {
      ...
      desc = m[0][4]
      fa    = "/export/EC1680U/Daniel_16S/16s/OUTPUT/B.5_OTU_cluster/" + id + ".trimmed.unique.fa"
      ...
      [desc, id, fa, table, align, taxon]
    }
  }

if (params.step == 'Otu_Similarity97')  {
   otu_output = list.map {
      it-> [it[0], it[1], file(it[2]), file(it[3]), file(it[4]), file(it[5])]
   }
}
...
process Rem_Chimeras {
  ...
  """
  export TERM=xterm
  /app_tools/mothur/mothur "#chimera.uchime(processors=${task.cpus}, fasta=${fna}, reference=${params.chimeras_ref})"
...
import glob
import os
#  fns = sorted(glob.glob("*.statistic_sample.txt")) # failed
...
process Upgma {
  ...
  #  weighted
  parallel_beta_diversity.py -i rarefaction/ -o weighted_unifrac/rare_dm/ -m weighted_unifrac -t /references/SILVA_132_release/trees/99/99_otus.tre -O ${task.cpus}
  ...
  parallel_beta_diversity.py -i rarefaction/ -o unweighted_unifrac/rare_dm/ -m unweighted_unifrac -t /references/SILVA_132_release/trees/99/99_otus.tre -O ${task.cpus}
