...
my $sample      = defined $ARGV[0]? shift @ARGV : '';
...
die "perl $0 [sampleid] [merged fasta] [cluster list] [classify otu] ([output file: statistic_sample.txt])\n" if(!-f $merged_fasta || !-f $cluster_list || !-f $classify_otu);
...
for my $i( 0 .. $#sample){
	...
	my @cache = map { m/^$sample[$i]\_.*/g } keys %fasta;
...
open(outFile, ">$output_file");
	...
	for my $i(0.. $#sample){
		...
		print outFile join("\t", $name, $totalTags{$name}, $uniqTags{$name}, $taxaTags{$name}, (scalar keys ($OTU{$name}))) ."\n";
	}
