...
//  memory 70.GB
  ...
  cp ${taxonomy} 97
  cd 97/
  /app_tools/mothur/mothur "#dist.seqs(processors=${task.cpus}, fasta=${align}, countends=F, cutoff=0)" > /dev/null
  /app_tools/mothur/mothur "#cluster(column=${sampleid}.trimmed.unique.dist, count=${count_table}, method=opti, cutoff=0)" > /dev/null
  /app_tools/mothur/mothur "#make.shared(list=${sampleid}.trimmed.unique.opti_mcc.list, count=${count_table})" > /dev/null
  /app_tools/mothur/mothur "#classify.otu(list=${sampleid}.trimmed.unique.opti_mcc.list, count=${count_table}, cutoff=80, label=0, taxonomy=$taxonomy)'" > /dev/null
...
#  filter_otus_from_otu_table.py -i ${sampleid}.otu_table.otuID.biom -o ${sampleid}.otu_table.otuID.filter.biom --min_count_fraction 0.00005 # ref: https://goo.gl/EMwR9N
  mv ${sampleid}.otu_table.otuID.biom ${sampleid}.otu_table.otuID.filter.biom
  ...
  /app_tools/mothur/mothur "#get.oturep(column= ${sampleid}.trimmed.unique.dist, list= ${sampleid}.trimmed.unique.opti_mcc.list, count=${count_table}, fasta=${fa}, method=abundance)" > /dev/null
...
#  	${sampleid}.otu_table.otuID.filter.dense.biom
  ...
  perl /export/EC1680U/perl/bin/16S/biomProc.pl idChangeUsingFasta \
  	...
  	${sampleid}.trimmed.unique.opti_mcc.0.rep.fasta,results.blast6
