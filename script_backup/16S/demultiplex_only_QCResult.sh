...
#do
	if [ ! -e "$MSFOLDER/${MSID}-16S.*.clean.fastq" ]; then
		/export/EC1680U/software/anaconda2/bin/pigz -cd -p 32 `ls ${MiseqClean}/*_R1_00*.clean.fastq.gz | grep -v unpaired` > "$MSFOLDER/${MSID}-16S.R1.clean.fastq"
		/export/EC1680U/software/anaconda2/bin/pigz -cd -p 32 `ls ${MiseqClean}/*_R2_00*.clean.fastq.gz | grep -v unpaired` > "$MSFOLDER/${MSID}-16S.R2.clean.fastq"
	fi;
#done | sort | uniq
...
cd 0_cleanFastq
ln -s $(ls $MSFOLDER/_demultiplexed_${MSID}-16S/*.clean.fastq | grep -v Undetermined) .
cd ..

ln -s ../MiSeq16Sanalysis_pipeline.v3v4cutadapt.pl .
ln -s ../V3FV4R_primer_V3V4.alignseq .
ls 0_cleanFastq/*.clean.fastq | cut -d'.' -f1 | cut -d'/' -f2 | sort -u > SampleName_16S.txt

echo ""
echo "perl MiSeq16Sanalysis_pipeline.v3v4cutadapt.pl -s SampleName_16S.txt -p V3FV4R_primer_V3V4.alignseq -r Y -o Y -i N 2>&1 | tee mothur.log"
echo ""
