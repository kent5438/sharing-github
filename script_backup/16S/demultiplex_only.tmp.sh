...
if [ ! -e "barcode_list/$MSID-16S-barcode_1.txt" ]; then
touch "barcode_list/$MSID-16S-barcode_1.txt"
echo "### ERROR: Please complete barcode list file!";
exit 1

else
rsync -av "barcode_list/$MSID-16S-barcode_1.txt" "$MSFOLDER"
...
/export/EC1680U/kentchen/miniconda3/envs/genomics-16S/bin/perl ../demultiplexFASTQ_byBarcodes.pl ${i}.R1.clean.fastq ${i}.R2.clean.fastq ${MSID}-16S-barcode_1.txt _demultiplexed_${i} _baseSpace_${i} 2>&1 | tee -a demultiplexFASTQ-${i}.pl.log
done
