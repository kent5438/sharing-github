...
my $docker_basal_cmd = "sudo docker run --rm -v `pwd`:`pwd` -v $fastq_path:$fastq_path -w `pwd` trinityrnaseq/trinityrnaseq:latest";
...
system("sudo chown $USER:users Trinotate.sqlite");
...
if(! -e "$anno_dir/transdecoder/Trinity.95.fasta.transdecoder.pep"){
	system("sh /export/EC1680U/perl/bin/Trinity/transdecoder_pipe.sh");
}
...
# my $gffread = "/export/EC1680U/software/cufflinks-2.2.1.Linux_x86_64/gffread";
# system("$gffread $anno_dir/transdecoder/Trinity.95.fasta.transdecoder.gff -T -o $anno_dir/transdecoder/Trinity.95.fasta.transdecoder.gtf");
if(! -e "$anno_dir/transdecoder/Trinity.95.fasta.transdecoder.gff"){
	system("mv $anno_dir/transdecoder/Trinity.95.fasta.transdecoder.gff3 $anno_dir/transdecoder/Trinity.95.fasta.transdecoder.gff");
}
my @samples = `cat $input | cut -f2`;
chomp(@samples);
foreach $sample (@samples){
    if(! -e "$quant_dir/$sample/bowtie2.sorted.bam"){
        system("picard SortSam I=$quant_dir/$sample/bowtie2.bam O=$quant_dir/$sample/bowtie2.sorted.bam SO=coordinate TMP_DIR=./tmp CREATE_INDEX=true");
    }

    if(! -e "$quant_dir/$sample/bowtie2.sorted_stats/qualimapReport.html"){
        system("qualimap bamqc -bam $quant_dir/$sample/bowtie2.sorted.bam -gff $anno_dir/transdecoder/Trinity.95.fasta.transdecoder.gff --java-mem-size=32G");
...
#my $blastx_cmd = "blastx -query $stat_dir/Trinity.95.fasta ".
#"-db $uniDB ".
...
#my $blastp_cmd = "blastp -query $anno_dir/transdecoder/Trinity.95.fasta.transdecoder.pep ".
#"-db $uniDB ".
...
my $blastx_cmd = "nextflow run /export/EC1680U/perl/bin/Trinity/blastx.nf -c /export/EC1680U/perl/bin/Trinity/nextflow.config -resume";
my $blastp_cmd = "nextflow run /export/EC1680U/perl/bin/Trinity/blastp.nf -c /export/EC1680U/perl/bin/Trinity/nextflow.config -resume";
...
#my $hmmer_cmd = "hmmscan --cpu 32 --domtblout $anno_dir/features/PFAM.out -E 0.01 ".
#"/export/EC1680U/DataBase/Trinotate/Pfam-A.hmm $transdec_pep 1> /dev/null";
my $hmmer_cmd = "nextflow run /export/EC1680U/perl/bin/Trinity/hmmscan.nf -c /export/EC1680U/perl/bin/Trinity/nextflow.config -resume";
...
# rnammer
my $rnammer_cmd = "RnammerTranscriptome.pl ".
...
my $get_contig_name = "grep '>' $stat_dir/Trinity.95.fasta | awk '{print \$1}' | sed 's/^>//g' > $stat_dir/contig1.txt.tmp";
system("$get_contig_name");
system("cp $stat_dir/contig1.txt.tmp $stat_dir/contig2.txt.tmp");
system("paste $stat_dir/contig1.txt.tmp $stat_dir/contig2.txt.tmp > $stat_dir/Trinity.95.fasta.gene_trans_map");
system("rm -rf $stat_dir/contig1.txt.tmp $stat_dir/contig2.txt.tmp");

my $loading_db = "Trinotate Trinotate.sqlite init ".
...
my $blastx2db = "Trinotate Trinotate.sqlite LOAD_swissprot_blastx $anno_dir/blast_result/uniprot.blastx.outfmt6";
my $blastp2db = "Trinotate Trinotate.sqlite LOAD_swissprot_blastp $anno_dir/blast_result/uniprot.blastp.outfmt6";
my $pfam2db = "Trinotate Trinotate.sqlite LOAD_pfam $anno_dir/features/PFAM.out";
my $signalp2db = "Trinotate Trinotate.sqlite LOAD_signalp $anno_dir/features/signalp.out";
my $tmhmm2db = "Trinotate Trinotate.sqlite LOAD_tmhmm $anno_dir/features/tmhmm.out";
my $rnammer2db = "Trinotate Trinotate.sqlite LOAD_rnammer $anno_dir/features/Trinity.95.fasta.rnammer.gff";
if(! -s "$anno_dir/Trinotate_report.xls"){system("rm -f $anno_dir/Trinotate_report.xls");}
if(! -e "$anno_dir/Trinotate_report.xls"){system("$loading_db; $blastx2db; $blastp2db; $pfam2db; $signalp2db; $tmhmm2db; $rnammer2db");}

my $trinotate_report = "Trinotate Trinotate.sqlite report > $anno_dir/Trinotate_report.xls";
if(! -e "$anno_dir/Trinotate_report.xls"){system("$trinotate_report");}

my $import_name = "import_transcript_names.pl Trinotate.sqlite $anno_dir/Trinotate_report.xls";
...
my $extract_go_terms = "extract_GO_assignments_from_Trinotate_xls.pl ".
...
my $go_slim_mapping = "Trinotate_GO_to_SLIM.pl ".
...
if($enrich_count != $comp_count){
	while(<IN>){
		...
		my $docker_mod_trans_cmd = "sudo docker run --rm -v $fastq_path:$fastq_path -v `pwd`:`pwd` -w `pwd`/$diff_dir/$comparisons kentchendocker/trinityrnaseq:latest";
		...
		system("sudo chown -R $USER:users $diff_dir/$comparisons");
    	...
    	"$diff_dir/$comparisons/*.DE_results $diff_dir/$comparisons/*.subset $diff_dir/$comparisons/*edgeR.count_matrix ". "$diff_dir/$comparisons/scatter* ".
...
while(<$ec_txt>){
	...
	foreach my $an_ec (@ec_list){
		open my $ec2kegg_txt, "<$kegg_dir/ec2kegg.txt";
        $hash{$trans} = $an_ec;
        while(my $line = <$ec2kegg_txt>){
            chomp($line);
            my @spp = split/\t/, $line;
...
if (! -e "$anno_dir/blast_result/uniprot.blastp.report.xlsx"){
	...
	print OUT "transcript_id\trefID\trefID_blastp\tpident\tlength\tqstart\tqend\tsstart\tsend\tevalue\tbitscore\tDescription\n";
		while(<IN>){
	    chomp;
	    my @sp = split/\t/, $_;
	    my @trans_sp = split/\.p1/, $sp[0];
	    my $trans_ID = $trans_sp[0];
	    print OUT "$trans_ID\t";
		my @refID_sp = split/\|/, $sp[1];
		my $ref_ID = $refID_sp[1];
		print OUT "$ref_ID\t";
	    for(my $i=1; $i<=$#sp; $i++){
	        print OUT "$sp[$i]\t";
	    }
...
if(! -e "$anno_dir/blast_result/uniprot.blastx.report.xlsx"){
	...
	print OUT "transcript_id\trefID\trefID_blastx\tpident\tlength\tqstart\tqend\tsstart\tsend\tevalue\tbitscore\tDescription\n";
	while(<IN>){
		chomp;
		my @sp = split/\t/, $_;
		print OUT "$sp[0]\t";
