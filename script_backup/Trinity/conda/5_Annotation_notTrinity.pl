...
system("export PERL5LIB=''"); # cannot find the bug that why PERL5LIB module of DB_file.pm will cause Transdecoder crash, thus empty PERL5LIB variable.
if(! -e "$anno_dir/transdecoder"){mkdir "$anno_dir/transdecoder";}
my $transdec_home = "/export/EC1680U/software/TransDecoder-3.0.1";
my $transdec_long_cmd = "$conda_home/TransDecoder.LongOrfs -t $stat_dir/Trinity.95.fasta";
my $transdec_predict_cmd = "$conda_home/TransDecoder.Predict -t $stat_dir/Trinity.95.fasta --cpu 32";
...
foreach $sample (@samples){
    ...
    if(! -e "$quant_dir/$sample/bowtie2.sorted_stats"){
        system("$qualimap_bamqc_cmd -bam $quant_dir/$sample/bowtie2.sorted.bam -gff $anno_dir/transdecoder/Trinity.95.fasta.transdecoder.gtf");
