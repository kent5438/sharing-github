...
sub report {
  ...
  if ($samplesize >= 3) {
	...
	#PCA
	$content = $content."<li><a href=files/clustvisPCA.pdf target=_blank>PCA plot</a></li>";
  }
...
sub inithtml {
  ...
  $cmd = "cp -r /export/EC1680U/Pipeline/Template/16S/css $output";
  push (@cmds, $cmd);
  $cmd = "cp -r /export/EC1680U/Pipeline/Template/16S/images/* $output/images";
  push (@cmds, $cmd);
  $cmd = "cp -r /export/EC1680U/Pipeline/Template/16S/DataAnalysisWorkflow.html $output/html/";
  push (@cmds, $cmd);
  $cmd = "cp -r /export/EC1680U/Pipeline/Template/16S/help.pdf $output/files/";
  push (@cmds, $cmd);
