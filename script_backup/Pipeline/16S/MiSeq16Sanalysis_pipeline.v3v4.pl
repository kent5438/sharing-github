...
for ($i=0;$i< $sCount ;$i++)
{
	...
	else 
	{
		...
		if ($primerSet =~ /V3V4/)
		{
			print "java -jar /export/arrayPRO1/commonSoftwares/trimmomatic-0.33.jar SE -threads 24 -phred33 -trimlog temp_$sampleIDs[$i].log ../$folder[2]/$sampleIDs[$i].$fastqSuffix[2] $sampleIDs[$i].$fastqSuffix[3] SLIDINGWINDOW:10:10 MINLEN:300\n";		
