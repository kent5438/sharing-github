...
```
File path: 4_Annotation/Prokka/
```
Transcriptome annotation is the process of identifying features of interest in a set of transcripts, and labelling them with useful information. “Prokka v1.12” is a software tool to annotate bacterial, archaeal and viral genomes quickly and produce standards-compliant output files [4].

As de-novo transcriptome finished, Prokka is a powerful tool which could help for finding bacteria origin of replication (ori), following with downstream gene/CDS/RNA sequence prediction and using blast method against the uniprot/swissprot bacterial database.

<br>

```{r, echo=FALSE, message=FALSE, warning=FALSE}
df <- read.table("files/4_Annotation/Prokka/prokka.txt", header=T, check.names=F, sep="\t")
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
df <- read.table("files/4_Annotation/Prokka/README.txt", header=T, check.names=F, sep="\t")
df$Extension <- paste0("[",df$Extension,"]","(files/4_Annotation/Prokka/prokka",df$Extension,")")
knitr::kable(df)
```
...
[Protein Function Table Download (.xlsx)](files/4_Annotation/prokka_func_anno.xlsx)

```{r, echo=FALSE, message=FALSE, warning=FALSE}
nucl_blast <- read_excel("files/4_Annotation/prokka_func_anno.xlsx", n_max = 10)
...
[KEGG Table Download (.xlsx)](files/4_Annotation/KEGG/prokka_ec2kegg.xls)
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
kegg_report <- read_excel("files/4_Annotation/KEGG/prokka_ec2kegg.xls")
...
## Final Summary Table (重要!)

[Transcript Functional Annotation Summary](files/final_report.xlsx)
