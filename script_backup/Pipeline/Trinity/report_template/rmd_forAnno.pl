...
while(my $line = <$rmd>){
    ...
    if($line =~ /###GO_COMPARISON###/){
        ...
        while(<IN>){
			...
			my $go_enrich_plot_all = `ls files/4_Annotation/GO/$comparisons\_enrich/*\.GOplot_dat\.pdf`;
			chomp($go_enrich_plot_all);
			my $go_enrich_subset_up = `ls files/4_Annotation/GO/$comparisons\_enrich/*$cases-UP\.subset\.GOseq\.enriched\.xlsx`;
			chomp($go_enrich_subset_up);
			my $go_enrich_subset_down = `ls files/4_Annotation/GO/$comparisons\_enrich/*$ctrls-UP\.subset\.GOseq\.enriched\.xlsx`;
			...
			$replace .= "[Sig UP (FDR<0.05 / log2FC >= 1)]\n\n";
            $replace .= "[$comparisons: UP (.xlsx)]($go_enrich_subset_up)\n\n";
            $replace .= "<br>\n\n";

			$replace .= "[Sig DOWN (FDR<0.05 / log2FC <= -1)]\n\n";
