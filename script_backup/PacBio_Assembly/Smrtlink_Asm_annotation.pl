...
my $task_root = "$smrt_root/jobs_root/003/$id/tasks";
my $mapping_dir = "$task_root/pbreports.tasks.mapping_stats_hgap-0";
my $coverage_dir = "$task_root/pbreports.tasks.coverage_report_hgap-0";
my $preassembly_dir = "$task_root/falcon_ns2.tasks.task_report_preassembly_yield-0";
my $polished_dir = "$task_root/pbreports.tasks.polished_assembly-0";
my $task_genome = "$task_root/falcon_ns2.tasks.task_falcon2_run_falcon_asm-0/p_ctg.fasta";
my $corrected_fasta = "$task_root/falcon_ns2.tasks.task_falcon2_run_falcon_asm-0/preads4falcon.fasta";
...
system("rsync -avq $corrected_fasta .");
...
my $circlator_cmd = "circlator all --assembler canu --threads 32 $genome preads4falcon.fasta circlator";
...
if(! -e "$assembly_dir/AssemblyStats"){
    mkdir("$assembly_dir/AssemblyStats");
}
# mapped subread stats
system("rsync -avq $mapping_dir/mapped_*_histogram.png $subread_dir");
system("rsync -avq $mapping_dir/mapped_concordance_vs_read_length.png $subread_dir");
# coverage report
system("rsync -avq $coverage_dir/coverage_histogram.png $assembly_dir/AssemblyStats");
# preassembly report
system("in2csv -k attributes $preassembly_dir/preassembly_yield.json | cut -d',' -f2,3 > $subread_dir/preassembly_yield.csv");
# polished assembly report
system("in2csv -k attributes $polished_dir/polished_assembly_report.json | cut -d',' -f2,3 > $assembly_dir/AssemblyStats/polished_assembly_report.csv");
system("rsync -avq $polished_dir/polished_coverage_vs_quality.* $assembly_dir/AssemblyStats");


### Prokka for gene prediction ###
print CurrTime() . "Processing prokka job...\n";
my $prokka_cmd = "prokka --outdir $prokka_dir --force --prefix $prefix --addgenes --kingdom Bacteria --gcode 11 --cpus 32 --rfam --rnammer $circular_genome";
...
system("sed -i '1d' $assembly_dir/quast_raw/report.tsv");
system("sed -i '1i Assembly\t$prefix' $assembly_dir/quast_raw/report.tsv");
...
system("sed -i '1d' $assembly_dir/quast_circ/report.tsv");
system("sed -i '1i Assembly\t$prefix' $assembly_dir/quast_circ/report.tsv");
...
#my $gilist = "/export/EC1680U/DataBase/gilist/bacteria/A_nosocomialis.prot.190304.gilist";
...
if($check_point == 0){
	system("sed -i '1i $prefix\_Contig\trefID\tpident\tlength\tqlen\tslen\tqstart\tqend\tsstart\tsend\tevalue\tbitscore\tdescription\ttaxID\tSciName\tkingdom\ttype' $anno_dir/$prefix\_contig_anno.txt");
}
...
system("rm -rf $report_dir/$assembly_dir/*.merge*");
...
# Copy report_template_smrtlink to current dir)
print CurrTime() . "Copy report_template_smrtlink to current dir...\n";
my $report_template_smrtlink = "/export/EC1680U/Pipeline/Pacbio_Denovo/report_template_smrtlink";
if(! -e "$report_template_smrtlink"){die "\n### ERROR: Please make sure '$report_template_smrtlink' existed!\n\n";}
system("rsync -avq $report_template_smrtlink .");

# Transfer 5_Report to report_template_smrtlink
print CurrTime() . "Transfer 5_Report/* to report_template_smrtlink/files...\n";
system("rsync -avq 5_Report/* report_template_smrtlink/files");

# Build R markdown Script
print CurrTime() . "Build Rmarkdown-Style Report...\n";
system("sed -i 's/<prefix>/$prefix/g' report_template_smrtlink/index.Rmd");
system("sed -i 's/<prefix>/$prefix/g' report_template_smrtlink/experiment.Rmd");
system("sed -i 's/<prefix>/$prefix/g' report_template_smrtlink/seqQC.Rmd");
system("sed -i 's/<prefix>/$prefix/g' report_template_smrtlink/assembly.Rmd");
system("sed -i 's/<prefix>/$prefix/g' report_template_smrtlink/annotation.Rmd");
system("sed -i 's/<prefix>/$prefix/g' report_template_smrtlink/tools.Rmd");
system("Rscript /export/EC1680U/Pipeline/Pacbio_Denovo/report_template_smrtlink/build_site.R");

# Rsync report to root folder & remove report_template_smrtlink
system("rsync -avq report_template_smrtlink/report .");
system("rm -rf report_template_smrtlink");
