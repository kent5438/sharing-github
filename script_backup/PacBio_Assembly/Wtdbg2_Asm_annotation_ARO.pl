...
my $corrected_fasta = `ls m54126_*.Q20.fasta`; chomp($corrected_fasta);
if(! -e "$corrected_fasta"){die "### ERROR: Please `ln -s` CCS Q20 fasta in $pwd!";}
...
if(! -e "$genome"){system("cp $task_genome $genome");}
...
my $circlator_cmd = "circlator all --assembler canu --threads 32 $genome $corrected_fasta circlator";
my $fixstart_cmd = "circlator fixstart circlator/06.fixstart.fasta circlator/$prefix\_polished_fixed";
if(! -e "$fixed_genome"){
    system("$circlator_cmd");
    system("sudo chown -R $USER:users circlator");
    system("$fixstart_cmd");
    system("rsync -av circlator/$prefix\_polished_fixed.fasta $fixed_genome");
}

if (! -e "$circular_genome"){
   my $circlator_minimus2 = "circlator minimus2 $fixed_genome $assembly_dir/$prefix\_polished_fixed";
   system("$circlator_minimus2");
   my $second_fixstart = "circlator fixstart $circular_genome $assembly_dir/tmp";
   system("$second_fixstart");
   system("mv $assembly_dir/tmp.fasta $circular_genome");
}
...
my $nrDB = "/export/EC1680U/DataBase/Uniprot/Bacteria/K_pneumoniae-taxonomy_573.fasta";
...
# Copy report_template_wtdbg2 to current dir)
print CurrTime() . "Copy report_template_wtdbg2 to current dir...\n";
my $report_template_wtdbg2 = "/export/EC1680U/Pipeline/Pacbio_Denovo/report_template_wtdbg2";
if(! -e "$report_template_wtdbg2"){die "\n### ERROR: Please make sure '$report_template_wtdbg2' existed!\n\n";}
system("rsync -avq $report_template_wtdbg2 .");

# Transfer 5_Report to report_template_wtdbg2
print CurrTime() . "Transfer 5_Report/* to report_template_wtdbg2/files...\n";
system("rsync -avq 5_Report/* report_template_wtdbg2/files");

# Build R markdown Script
print CurrTime() . "Build Rmarkdown-Style Report...\n";
system("sed -i 's/<prefix>/$prefix/g' report_template_wtdbg2/index.Rmd");
system("sed -i 's/<prefix>/$prefix/g' report_template_wtdbg2/experiment.Rmd");
system("sed -i 's/<prefix>/$prefix/g' report_template_wtdbg2/seqQC.Rmd");
system("sed -i 's/<prefix>/$prefix/g' report_template_wtdbg2/assembly.Rmd");
system("sed -i 's/<prefix>/$prefix/g' report_template_wtdbg2/annotation_ARO.Rmd");
system("sed -i 's/<prefix>/$prefix/g' report_template_wtdbg2/tools.Rmd");
system("Rscript /export/EC1680U/Pipeline/Pacbio_Denovo/report_template_wtdbg2/build_site_ARO.R");

# Rsync report to root folder & remove report_template_wtdbg2
system("rsync -avq report_template_wtdbg2/report .");
system("rm -rf report_template_wtdbg2");
