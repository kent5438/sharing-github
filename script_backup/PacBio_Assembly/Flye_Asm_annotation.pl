...
system(". activate pb-rsii-denovo");
...
my $scaffolds_fasta = "flye/pbjelly_out/$prefix.scaffolds.polished.fasta";
if(! -e "$scaffolds_fasta"){die "### ERROR: Please make sure you have completed LINKS/pbjelly/pbmm2/arrow pipeline!";}
...
if (! -e "$circular_genome"){
    my $circlator_minimus2 = "circlator minimus2 $scaffolds_fasta circlator/minimus2";
...
my $quast_cmd = "quast -o $assembly_dir/quast $contigs_fasta $scaffolds_fasta $fna";
...
system("sed -i 's/<prefix>/$prefix/g' report_template_flye/annotation.Rmd");
system("sed -i 's/<prefix>/$prefix/g' report_template_flye/tools.Rmd");
system("Rscript /export/EC1680U/Pipeline/Pacbio_Denovo/report_template_flye/build_site.R");
