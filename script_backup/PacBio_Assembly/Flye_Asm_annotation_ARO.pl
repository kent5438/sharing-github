...
# copy contigs fasta
my $contigs_fasta = "flye/assembly.fasta";
my $contigs_polished_fasta = "flye/$prefix.contigs.polished.fasta";
if(! -e "$contigs_fasta"){die "### ERROR: Please make sure you have completed Flye pipeline!";}
system("cp $contigs_fasta $assembly_dir/$prefix.contigs.fasta");

# copy scaffolds polished fasta
my $scaffolds_fasta = "flye/PacBio_scaffolder_results/pbjelly_out/$prefix.scaffolds.polished.fasta";
if((! -e "$scaffolds_fasta" and (-e "$contigs_polished_fasta"))) {
    ...
    die "### ERROR: Please make sure you have completed any of SSPACE-Long/pbjelly/pbmm2/arrow pipeline!";
}
...
system("perl -pe 's/^#//' flye/assembly_info.txt | cut -f1,2,3,4 > $assembly_dir/assembly_info.txt");
...
if (! -e "$circular_genome"){
    if ($num_of_contigs != 1) {
		my $circlator_clean = "circlator clean $assembly_dir/$prefix.scaffolds.polished.fasta circlator/clean --verbose";
		my $circlator_minimus2 = "circlator minimus2 circlator/clean.fasta circlator/clean.minimus2";
		my $circlator_fixstart = "circlator fixstart circlator/clean.minimus2.circularise.fasta circlator/clean.minimus2.circularise.fixed --verbose";
        system("$circlator_clean");
		system("$circlator_minimus2");
		system("$circlator_fixstart");
        system("rsync -avq circlator/clean.minimus2.circularise.fixed.fasta $circular_genome");
    } else {
		my $circlator_clean = "circlator clean $assembly_dir/$prefix.scaffolds.polished.fasta circlator/clean --verbose";
        my $circlator_fixstart = "circlator fixstart circlator/clean.fasta circlator/clean.fixed --verbose";
		system("$circlator_clean");
        system("$circlator_fixstart");
        system("rsync -avq circlator/clean.fixed.fasta $circular_genome");
    }
}
...
my $nrDB = "/export/EC1680U/DataBase/Uniprot/Bacteria/Pseudomonas_aeruginosa_287_200610.fasta";
...
# Copy report_template_flye to current dir)
print CurrTime() . "Copy report_template_flye to current dir...\n";
my $report_template_flye = "/export/EC1680U/Pipeline/Pacbio_Denovo/report_template_flye";
if(! -e "$report_template_flye"){die "\n### ERROR: Please make sure '$report_template_flye' existed!\n\n";}
system("rsync -avq $report_template_flye .");

# Transfer 5_Report to report_template_flye
print CurrTime() . "Transfer 5_Report/* to report_template_flye/files...\n";
system("rsync -avq 5_Report/* report_template_flye/files");

# Build R markdown Script
print CurrTime() . "Build Rmarkdown-Style Report...\n";
system("sed -i 's/<prefix>/$prefix/g' report_template_flye/index.Rmd");
system("sed -i 's/<prefix>/$prefix/g' report_template_flye/experiment.Rmd");
system("sed -i 's/<prefix>/$prefix/g' report_template_flye/seqQC.Rmd");
system("sed -i 's/<prefix>/$prefix/g' report_template_flye/assembly.Rmd");
system("sed -i 's/<prefix>/$prefix/g' report_template_flye/annotation_ARO.Rmd");
system("sed -i 's/<prefix>/$prefix/g' report_template_flye/tools.Rmd");
system("sed -i 's/<prefix>/$prefix/g' report_template_flye/report.Rmd");
system("/export/EC1680U/kentchen/miniconda3/envs/pandoc/bin/Rscript /export/EC1680U/Pipeline/Pacbio_Denovo/report_template_flye/build_site_ARO.R");

# Rsync report to root folder & remove report_template_flye
system("rsync -avq report_template_flye/report.pdf report_template_flye/report");
system("rsync -avq report_template_flye/report .");
system("rm -rf report_template_flye");
