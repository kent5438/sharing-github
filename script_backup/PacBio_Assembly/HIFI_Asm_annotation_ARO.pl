...
my @folders = qw(1_Assembly 2_HiFiReads 3_Prokka 4_Annotation 5_Report);
my ($assembly_dir, $hifi_dir, $prokka_dir, $anno_dir, $report_dir) = @folders;
...
# copy contigs fasta
my $contigs_fasta = "hifi/hifiasm.p_ctg.fasta";
my $contigs_polished_fasta = "hifi/$prefix.contigs.polished.fasta";
if(! -e "$contigs_fasta"){die "### ERROR: $contigs_fasta is not found!\n
Please make sure you have completed HIFI pipeline!\n";}
system("cp $contigs_fasta $assembly_dir/$prefix.contigs.fasta");

# copy scaffolds polished fasta
my $scaffolds_fasta = "hifi/PacBio_scaffolder_results/pbjelly_out/$prefix.scaffolds.polished.fasta";
if((! -e "$scaffolds_fasta") and (-e "$contigs_polished_fasta")) {
    system("cp $contigs_polished_fasta $assembly_dir/$prefix.scaffolds.polished.fasta");
} elsif ((-e "$scaffolds_fasta") or (-e "$contigs_polished_fasta")) {
    system("cp $scaffolds_fasta $assembly_dir/$prefix.scaffolds.polished.fasta");
} elsif ((! -e "$scaffolds_fasta") and (! -e "$contigs_polished_fasta")) {
    die "### ERROR: each of $contigs_polished_fasta or $scaffolds_fasta is not found!\n
Please make sure you have completed any of SSPACE-Long/pbjelly/pbmm2/arrow pipeline!\n";
}

if(! -e "$scaffolds_fasta"){
    system("cp $contigs_polished_fasta $assembly_dir/$prefix.scaffolds.polished.fasta");
}
...
my $fixed_genome = "circlator/06.fixstart.fasta";
...
#my $circlator_cmd = "$docker_basal_cmd all --threads 32 --genes_fa RecA.fasta $scaffolds_fasta corrected.fasta circlator";
#####


my $num_of_contigs = `grep -c '>' $assembly_dir/$prefix.scaffolds.polished.fasta`; chomp($num_of_contigs);


#### check canu corrected process is completed or not
if(! -e "canu_corrected/$prefix.correctedReads.fasta.gz"){
    system("/export/EC1680U/kentchen/miniconda3/envs/canu/bin/canu -correct -d canu_corrected -p $prefix -pacbio-raw subreads/$prefix.subreads.fastq useGrid=true genomeSize=5m gridEngine='slurm' gridOptions='--partition=wgs' corOverlapper=minimap");
}

if (! -e "$fixed_genome"){
    if(-e "circlator"){system("rm -rf circlator");}
    system("/export/EC1680U/kentchen/opt/circlator-1.5.5-docker4/scripts/circlator all $assembly_dir/$prefix.scaffolds.polished.fasta canu_corrected/$prefix.correctedReads.fasta.gz circlator --threads 32 --bwa_opts '' --assembler canu");
    system("rsync -avq circlator/06.fixstart.fasta $circular_genome");
}

#### Create Circularized Info
print CurrTime() . "Create Circlator assembly_info.txt...\n";
my $circ_bam = "circlator/01.mapreads.bam";
my $circ_info = "circlator/04.merge.circularise.log";
if((! -e $circ_info) or (! -e $circ_bam)){die "### ERROR: Please make sure 'circlator' is running complete or not!\n";}
my $mosdepth = "/export/EC1680U/kentchen/miniconda3/envs/mosdepth/bin/mosdepth";
system("$mosdepth tmp $circ_bam -n -t 32");
system('cat tmp.mosdepth.summary.txt | cut -f1,2,4 | sed -e "s/chrom/seq_name/" | sed -e "s/mean/cov./" | sed "\$d" > test1.txt');
system('cut -f6 circlator/04.merge.circularise.log > test2.txt');

system("paste test1.txt test2.txt | sed -e 's/circularised/circ./' > circlator/assembly_info.txt");
system("cp circlator/assembly_info.txt $assembly_dir/assembly_info.txt");


### Transfer HIFI reads ###
print CurrTime() . "Transfer HIFI reads...\n";
...
my $quast_cmd = "quast -o $assembly_dir/quast $contigs_fasta $assembly_dir/$prefix.scaffolds.polished.fasta $fna";
if(! -e "$assembly_dir/quast/report.tsv"){
    system("$quast_cmd");
}

system("sed -i '1d' $assembly_dir/quast/report.tsv");
system("sed -i '1i $prefix\tcontigs\tscaffolds-polished\tcirclator' $assembly_dir/quast/report.tsv");
...
my $check_GO = `grep 'GO_ID' $go_dir/GO_mapping.txt`; chomp($check_GO);
if(! $check_GO){
...
my $nrDB = "/export/EC1680U/DataBase/Uniprot/Swiss-Prot/uniprot-reviewed_20200512.fasta";
...
if(! -e "$anno_dir/$prefix\_contig_anno.txt.tmp"){
    my $blastn_cmd = "/export/EC1680U/kentchen/miniconda3/envs/general/bin/blastn -query $fna -db $ntDB -num_threads 32 ".
...
## BLAST to VFDB Database
print CurrTime() . "BLAST to VFDB Database...\n";
my $vfdb_txt = "$anno_dir/$prefix\_blastp\.vfdb\.txt";
my $vfdb_xlsx = "$anno_dir/$prefix\_blastp\.vfdb\.xlsx";
my $vfdb_DB = "/export/EC1680U/DataBase/VFDB/VFDB_setB_pro.faa";
if(! -e "$vfdb_txt"){
	system("blastp -query $faa -db $vfdb_DB -num_threads 32 -max_target_seqs 1 -max_hsps 1 -outfmt '6 qseqid evalue stitle' -out $vfdb_txt");
}
$check_line = `grep 'vfdb_evalue' $vfdb_txt`;
chomp($check_line);
if(! $check_line){
    my $cmd = "sed -i '1i Prokka_ID\tvfdb_evalue\tvfdb_anno' $vfdb_txt";
    system("$cmd");
}
system("tab2xlsx.pl $vfdb_txt $vfdb_xlsx");
...
system("grep -v '##' $prokka_dir/$prefix.gff | grep -P '^[0-9]+F|^gnl|^[0-9]+|^Contig' | grep -P -v \"\tgene\t\" > $prokka_dir/$prefix.gff.tmp");
...
$report_cmd .= 'vfdb <- read.table("'.$vfdb_table.'", header=T, sep="\t", check.names = FALSE, fill=TRUE, quote="\"");';
...
$report_cmd .= 'merge7 <- merge(merge6, vfdb, by="Prokka_ID", all.x=TRUE);';
$report_cmd .= 'merge8 <- merge(merge7, card, by="Prokka_ID", all.x=TRUE);';
$report_cmd .= 'write.table(merge8, file="'.$report_out.'", sep="\t", quote=F, na="-", row.names = FALSE);';
...
### add CARD visualization graph
print CurrTime() . "CARD visualization...\n";
system('cat '."$anno_dir/$prefix\_func_anno.txt".' | egrep "Perfect|Strict" | cut -f29 | sort -V | uniq -c | perl -pe "s/^\s+//g" | perl -pe "s/\s/,/" | sort -k1,1 -r -V | grep -v "-" > '."$anno_dir/drug_class.sig.plotly.txt");
system('cat '."$anno_dir/$prefix\_func_anno.txt".' | egrep "Perfect|Strict" | cut -f30 | sort -V | uniq -c | perl -pe "s/^\s+//g" | perl -pe "s/\s/,/" | sort -k1,1 -r -V | grep -v "-" > '."$anno_dir/resistance_mechanism.sig.plotly.txt");
system('cat '."$anno_dir/$prefix\_func_anno.txt".' | egrep "Perfect|Strict" | cut -f31 | sort -V | uniq -c | perl -pe "s/^\s+//g" | perl -pe "s/\s/,/" | sort -k1,1 -r -V | grep -v "-" > '."$anno_dir/AR_family.sig.plotly.txt");
system("/export/EC1680U/kentchen/miniconda3/envs/python3/bin/python /export/EC1680U/perl/bin/Pacbio/pie_chart.py");




### Report $assembly_dir, $hifi_dir, $prokka_dir, $anno_dir, $report_dir
...
system("rsync -avq $hifi_dir $report_dir");
system("rsync -avq --exclude='*tmp*' --exclude='*oneline*' --include='*.csv' $prokka_dir $report_dir");
system("rsync -avq -m --include='*.xlsx' --include='*.pdf' --include='*.png' --include='*.xls' --include='*.csv' --include='*.html' --include='*/' --exclude='*' --exclude='*tmp*' --exclude='*oneline*' $anno_dir $report_dir");
...
# Copy report_template_hifi to current dir)
print CurrTime() . "Copy report_template_hifi to current dir...\n";
my $report_template_hifi = "/export/EC1680U/Pipeline/Pacbio_Denovo/report_template_hifi";
if(! -e "$report_template_hifi"){die "\n### ERROR: Please make sure '$report_template_hifi' existed!\n\n";}
system("rsync -avq $report_template_hifi .");

# Transfer 5_Report to report_template_hifi
print CurrTime() . "Transfer 5_Report/* to report_template_hifi/files...\n";
system("rsync -avq 5_Report/* report_template_hifi/files");

# Build R markdown Script
print CurrTime() . "Build Rmarkdown-Style Report...\n";
system("sed -i 's/<prefix>/$prefix/g' report_template_hifi/index.Rmd");
system("sed -i 's/<prefix>/$prefix/g' report_template_hifi/experiment.Rmd");
system("sed -i 's/<prefix>/$prefix/g' report_template_hifi/seqQC.Rmd");
system("sed -i 's/<prefix>/$prefix/g' report_template_hifi/assembly.Rmd");
system("sed -i 's/<prefix>/$prefix/g' report_template_hifi/annotation_ARO.Rmd");
system("sed -i 's/<prefix>/$prefix/g' report_template_hifi/tools.Rmd");
system("sed -i 's/<prefix>/$prefix/g' report_template_hifi/report.Rmd");
system("/export/EC1680U/kentchen/miniconda3/envs/pandoc/bin/Rscript /export/EC1680U/Pipeline/Pacbio_Denovo/report_template_hifi/build_site_ARO.R");

# Rsync report to root folder & remove report_template_hifi
system("rsync -avq report_template_hifi/report.pdf report_template_hifi/report");
system("rsync -avq report_template_hifi/report .");
system("rm -rf report_template_hifi blastp.tmp");
