...
sub CalculateExpression {
  ...
  foreach $sample (@samples) {    
    ...
    $cmd=$cmd."$rsem_root/rsem-calculate-expression --bowtie2 -p 20 --paired-end --forward-prob 0 --output-genome-bam --phred33-quals $r1 $r2 $reference $sample \n";          	
	$cmd=$cmd."mv $sample* $out_CE\n";
    unless (-e $err) {create_fullshell ($shell, $qname, $err, $log, $cmd);  `qsub -q all.q $shell >> ./jobs` ;}
  }
  checkjobid("");
...
# SET1:T24-1ug;T24
  foreach $set (sort keys %{$parameters}) {
	...
	unless (-e $err) {create_fullshell ($shell, $qname, $err, $log, $cmd);   `sh $shell >> ./jobs`  ;}		
  }   
  checkjobid("");
}
sub ebseq {  
...
#  my @samples;   
  ...
  # All samples heatmap plot by merged FPKM	
    ...
    system("/usr/bin/Rscript" . " -e '" . $merge_cmd . "'");

    system("/usr/bin/Rscript /export/EC1680U/Rscript/Trinity/heatmap_eachSample.R $out_CE/all_samples_FPKM.txt $report_out/images");
...
#      $tmp = $tmp."$out_ebseq/$comparsion/$comparsion.genMat.fdr-0.05.result#";
      $tmp = $tmp."$out_ebseq/$comparsion/$comparsion.genMat.result.1#";
    }    
    chop ($tmp);	
    $cmd = "/export/EC1680U/kentchen/DGE/heatmap.kentchen.pl --comps=$tmp --output=$stats_out";
    unless (-e "$stats_out/comparsion.txt") {process_cmd($cmd);}
    $cmd = "/export/EC1680U/kentchen/DGE/heatmap_union.pl --comps=$tmp --output=$stats_out";
    unless (-e "$stats_out/comparsion_union.txt") {process_cmd($cmd);}
    
	system("sed -i 's/\.genMat\.result\.1//g' $stats_out/comparsion.txt");
	system("sed -i 's/\.genMat\.result\.1//g' $stats_out/comparsion_union.txt");
    		
    $cmd = "Rscript $rscript/heatmap_rowname.r $stats_out/comparsion.txt $report_out/images/";
    unless (-e "$report_out/images/heatmap.pdf") {process_cmd($cmd);}
    $cmd = "Rscript /export/EC1680U/kentchen/DGE/heatmap_union.r $stats_out/comparsion_union.txt $report_out/images/";
    unless (-e "$report_out/images/heatmap_union.pdf") {process_cmd($cmd);}		
    $cmd = "$perlbin_root/tab2xls.pl $stats_out/comparsion.txt $report_out/files/comparsion.xls";
    unless (-e "$report_out/files/comparsion.xls") {process_cmd($cmd);}		
    $cmd = "$perlbin_root/tab2xls.pl $stats_out/comparsion_union.txt $report_out/files/comparsion_union.xls";
    unless (-e "$report_out/files/comparsion_union.xls") {process_cmd($cmd);} 

    $cmd = "sed 's|Heatmap_Samples|<li><a href=images/all_samples_heatmap.pdf target=display>Samples HeatMap</a><a style=\"color: blue; background-color: transparent; text-decoration: none;\" href=files/all_samples_FPKM.xls target=_blank>(.xls)</a></li>|' $index_template > $stats_out/index_template\.1";
    process_cmd ($cmd);
    $cmd = "sed -i 's|Heatmap_intersection|<li><a href=images/heatmap.pdf target=display>HeatMap (Intersection)</a><a style=\"color: blue; background-color: transparent; text-decoration: none;\" href=files/comparsion.xls target=_blank>(.xls)</a></li>|' $stats_out/index_template\.1";
    process_cmd ($cmd);
    $cmd = "sed -i 's|Heatmap_union|<li><a href=images/heatmap_union.pdf target=display>HeatMap (Union)</a><a style=\"color: blue; background-color: transparent; text-decoration: none;\" href=files/comparsion_union.xls target=_blank>(.xls)</a></li>|' $stats_out/index_template\.1";
    process_cmd ($cmd);
  } else {
  ...
  foreach $set (sort keys %{$parameters}) {
    ...
    $cmd=$cmd."Rscript $rscript/clusterProfiler.over-representation.v3.1.kentchen.r $out_ebseq/$case-$ctrl/$case-$ctrl.genMat.fdr-0.05.result $genome $output \n";
    ...
    $cmd=$cmd."$perlbin_root/fastq-stats.pl --samplelist=$tmp --readdir=$dir/reads --fastqstats_root=$fastqstats_root --stats_out=$stats_out --fastqstats_template=$fastqstats_template --outputdir=$output\n";		
    ...
    $cmd=$cmd."$perlbin_root/rsem-stats.pl --samplelist=$tmp  --calexpdir=$out_CE --samstats_template=$samstats_template --outputdir=$output\n";	
...
sub inithtml {
  ...
  push (@cmds, $cmd);
  $cmd = "cp -r /export/EC1680U/kentchen/Template/DGE/help.pdf $output/html";
  push (@cmds, $cmd);
