...
# All samples correlation by merged FPKM
	...
	system("Rscript /export/EC1680U/Rscript/Trinity/correlation_allSample.R $out_CE/all_samples_FPKM.txt $report_out/images");
...
#    $cmd = "$perlbin_root/heatmap.pl --comps=$tmp --output=$stats_out";
    ...
    if (! -e "$stats_out/comparsion.txt") {process_cmd($cmd);}
    $cmd = "/export/EC1680U/kentchen/DGE/heatmap_union.pl --comps=$tmp --output=$stats_out";
    if (! -e "$stats_out/comparsion_union.txt") {process_cmd($cmd);}
