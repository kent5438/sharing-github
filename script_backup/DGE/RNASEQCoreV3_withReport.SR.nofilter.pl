...
#	$cmd=$cmd."$rsem_root/rsem-control-fdr $out_ebseq/$case-$ctrl/$case-$ctrl.genMat.result 0.3 $out_ebseq/$case-$ctrl/$case-$ctrl.genMat.C1.5.result \n";
#	$cmd=$cmd."$rsem_root/rsem-control-fdr $out_ebseq/$case-$ctrl/$case-$ctrl.isoMat.result 0.3 $out_ebseq/$case-$ctrl/$case-$ctrl.isoMat.C1.5.result \n";
	unless (-e $err) {create_fullshell ($shell, $qname, $err, $log, $cmd);   `sh $shell >> ./jobs` ;}
  }     
  checkjobid("");
}
...
#      $tmp = $tmp."$out_ebseq/$comparsion/$comparsion.genMat.fdr-0.05.result#";
    ...
    chop ($tmp);	
#    $cmd = "$perlbin_root/heatmap.pl --comps=$tmp --output=$stats_out";
	$cmd = "/export/EC1680U/kentchen/DGE/heatmap.kentchen.nofilter.pl --comps=$tmp --output=$stats_out";
...
#    $cmd=$cmd."Rscript $rscript/clusterProfiler.over-representation.v3.1.kentchen.r $out_ebseq/$case-$ctrl/$case-$ctrl.genMat.fdr-0.05.result $genome $output \n";
	$cmd=$cmd."Rscript $rscript/clusterProfiler.over-representation.v3.1.cutoff010.r $out_ebseq/$case-$ctrl/$case-$ctrl.genMat_C1.2.result $genome $output \n";
    ...
    $cmd=$cmd."$perlbin_root/rsem-stats.pl --samplelist=$tmp  --calexpdir=$out_CE --samstats_template=$samstats_template --single-end --outputdir=$output\n";	
