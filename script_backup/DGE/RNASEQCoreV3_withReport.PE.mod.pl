...
sub CalculateExpression {
  ...
  foreach $sample (@samples) {    
    ...
    $cmd = "";    
    $r1 = "$dir/reads/$sample.R1.clean.fastq.gz";
    $r2 = "$dir/reads/$sample.R2.clean.fastq.gz";	
    ...
    $cmd=$cmd."$rsem_root/rsem-calculate-expression --paired-end --bowtie2 -p 20 --output-genome-bam --phred33-quals $r1 $r2 $reference $sample \n";          	
