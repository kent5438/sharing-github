...
if(! -e "fastqc_result"){
	mkdir("fastqc_result");
	system("fastqc -t 32 reads/*.fastq* -o fastqc_result");
}
...
my $quast = "python2 /export/EC1680U/kentchen/miniconda3/envs/pb-rsii-denovo/bin/quast";
...
system("fastalength".' '.$contigs.' | sed \'s/ /\t/g\' | awk \'{ t=$1; $1=$2; $2=t; print; }\' | sed \'s/ /\t/g\' > fastalength.tmp');

# convert a multi-line fasta to a single-line fasta
my $oneline_fa = 'awk \'/^>/ {printf("\n%s\n",$0);next; } { printf("%s",$0);}  END {printf("\n");}\' < '.$contigs.' > '.$contigs.'.oneline.fasta';
system("$oneline_fa");
system("sed -i '/^ *\$/d' $contigs.oneline.fasta"); # remove empty line


# split one single-line fasta into multiple single fasta files
my $split_fa = 'cat '.$contigs.'.oneline.fasta | awk \'/^>/ {if(N>0) printf("\n"); printf("%s\n",$0);++N;next;} { printf("%s",$0);} END {printf("\n");}\' | split -l 2 -a 3 - seq_';
system("$split_fa");
...
my $gilist = "/export/EC1680U/DataBase/gilist/Ecoli.prot.180127.gilist"; # all bacteria gilist
...
if(! -e "$anno_dir/$prefix\_contig_anno.txt.tmp"){
	...
	"-outfmt '6 qseqid sseqid pident length qlen slen qstart qend sstart send evalue score stitle qseq staxids sscinames sskingdoms' -evalue 1e-10 -max_target_seqs 3 -max_hsps 1 -out $anno_dir/$prefix\_contig_anno.txt.tmp";
    system("$blastn_cmd");
}
...
if($check_point == 0){
	system("sed -i '1i $prefix\_Contig\tNCBI_ID\tpident\tlength\tqlen\tslen\tqstart\tqend\tsstart\tsend\tevalue\tbitscore\tdescription\tqseq\ttaxID\tSciName\tkingdom\tcoverage\ttype' $anno_dir/$prefix\_contig_anno.txt");
}
...
if(! -e "$faa.blastp.tmp"){
	system("blastp -query $faa -db $nrDB -num_threads 32 -gilist $gilist -max_target_seqs 1 -max_hsps 1 -outfmt '6 qseqid stitle' -out $faa.blastp.tmp");
}
