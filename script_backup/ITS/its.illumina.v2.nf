...
process Join_Fastq {
  ...
  /export/EC1680U/software/pear-0.9.10-bin-64/pear -m 600 -n 50 -j 32 -o ${sampleid} -f ${trim_r1} -r ${trim_r2} 2>&1 | tee ${sampleid}.log
  cp ${sampleid}.assembled.fastq ${sampleid}.extendedFrags.fastq
  """
}
...
#  /app_tools/mothur/mothur "#chimera.uchime(processors=${task.cpus}, fasta=${fna}, reference=${params.its_chimeras_ref})"
  /export/EC1680U/kentchen/opt/usearch11.0.667_i86linux32 -uchime2_ref ${fna} -db ${params.its_chimeras_ref} -strand plus -threads ${task.cpus} -mode high_confidence -chimeras ${sampleid}.ref.uchime.chimeras -notmatched ${sampleid}.trimmed.fa
...
process Otu_Preprocess {
  ...
  /app_tools/mothur/mothur "#classify.seqs(processors=${task.cpus}, fasta=${sampleid}.trimmed.unique.fa, name=${sampleid}.trimmed.names, template=${params.unite_otu_ref_fasta}, taxonomy=${params.unite_otu_ref_taxonomy}, cutoff=80, method=wang)"  # 輸出 XX.trimmed.unique.raw_taxonomy.wang.taxonomy  & XX.trimmed.unique.raw_taxonomy.wang.tax.summary
    # Coral_4.trimmed.unique.UNITEv6_sh_99.wang.taxonomy
    # Coral_4.trimmed.unique.UNITEv8_sh_dynamic.wang.taxonomy
  /app_tools/mothur/mothur "#remove.lineage(fasta=${sampleid}.trimmed.unique.fa, count=${sampleid}.trimmed.count_table, taxonomy=${sampleid}.trimmed.unique.UNITEv8_sh_dynamic.wang.taxonomy, taxon=Chloroplast-Mitochondria-unknown-mitochondria-chloroplast)" > /dev/null # 輸出 XX.trimmed.unique.raw_taxonomy.wang.pick.taxonomy & XX.trimmed.unique.pick.fa & XX.trimmed.pick.count_table
  ...
  mv ${sampleid}.trimmed.unique.UNITEv8_sh_dynamic.wang.pick.taxonomy ${sampleid}.trimmed.unique.unite.wang.taxonomy
...
process Otu_Similarity97 {
  ...
  vsearch --usearch_global ${sampleid}.trimmed.unique.opti_mcc.0.03.rep.fasta --db ${params.unite_otu_ref_fasta} --id 0.9 --blast6out ${sampleid}.results.blast6 --maxaccepts 1 --threads 32
