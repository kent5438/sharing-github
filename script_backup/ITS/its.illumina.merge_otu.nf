...
demultiplex_quality_filter_output2 = Channel
  ...
  .map { row -> 
    ...
    fa = "OUTPUT/B.3_demultiplex_quilty_filter/" + row.SampleID + ".fna"

    [row.Description, row.SampleID, fa]
  //}.set{ sample_reads_qc, sample_reads_trim }
  }

(otu_output_similarity971, otu_output_similarity972) = Channel
  .from(mapping_file)
  .splitCsv(sep: "\t", header: ['SampleID','BarcodeSequence','LinkerPrimerSequence','Description','FilePath'], strip: true, skip: 1)
  .map { row ->
    list  = "OUTPUT/B.5_OTU_cluster/97/" + row.SampleID + ".trimmed.unique.opti_mcc.list"
    tax   = "OUTPUT/B.5_OTU_cluster/97/" + row.SampleID + ".trimmed.unique.opti_mcc.0.03.cons.taxonomy"
    biom  = "OUTPUT/B.5_OTU_cluster/97/" + row.SampleID + ".otu_table.taxID.filter.dense.dedup.biom"
    txt   = "OUTPUT/B.5_OTU_cluster/97/" + row.SampleID + ".otu_table.taxID.filter.dense.dedup.txt"

    [row.Description, row.SampleID, file(list), file(tax), file(biom), file(txt)]
  }.into(2)
