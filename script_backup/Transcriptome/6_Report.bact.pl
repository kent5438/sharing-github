...
my $anno_report = "$anno_dir/prokka_func_anno.txt";
...
$report_cmd .= 'anno <- read.table("'.$anno_report.'", header=T, sep="\t", check.names = FALSE, fill=TRUE, quote="\"");';
...
$report_cmd .= 'merge2 <- merge(merge1, anno, by="transcript_id", all.x=TRUE);';
#$report_cmd .= 'merge2 <- merge(merge1, blastp, by="transcript_id", all.x=TRUE);';
#$report_cmd .= 'merge3 <- merge(merge2, blastx, by="transcript_id", all.x=TRUE);';
...
$report_cmd .= 'write.table(merge2, file="final_report.txt.tmp", sep="\t", quote=F, na="-", row.names = FALSE);';
...
system("rsync -avq -m --exclude='*tmp*' --exclude='*oneline*' --include='*.csv' $anno_dir $report_dir");
#system("rsync -avq --exclude='Trinity.95.fasta.transdecoder_dir*/' --exclude='work/' $anno_dir/Prokka $report_dir/$anno_dir/Gene_prediction");
...
system("Rscript /export/EC1680U/Pipeline/Trinity/build_site.bact.R");
chdir("..");
