...
my $quant_folder_count = `ls $quant_dir | grep -v report | wc -l`; chomp($quant_folder_count);
my $sample_count = `wc -l $input`; chomp($input);
...
while(<$sample_in>){
	...
	my $sorted_bam = "/export/EC1680U/software/anaconda2/bin/samtools sort -@ 32 $quant_sample_dir/bowtie2.bam $quant_sample_dir/bowtie2.sorted.bam";
	system("$sorted_bam");
	my $sorted_bam_index = "/export/EC1680U/software/anaconda2/bin/samtools index $quant_sample_dir/bowtie2.sorted.bam";
