...
if [ ! -e $INPUT ]; then 
	...
	echo "$0 -i $INPUT -c $COMP -k $KEGG -t $TYPE" >> "CMD_history.txt"
fi
...
### Run Pipeline
if [ ! -d "trinity_out_dir/Trintiy.95.fasta" ]; then
...
if [ ! -d "1_AssemblyStats" ]; then
	/export/EC1680U/perl/bin/Trinity/2_Trinity_interval_count_notTrinity.pl -i $INPUT 2>&1 | tee log/2_Trinity_interval_count.log
fi

if [ ! -d "2_Quantitation" ]; then
	/export/EC1680U/perl/bin/Trinity/3_Transcript_exp_quant_notTrinity.pl -i $INPUT 2>&1 | tee log/3_Transcript_exp_quant.log
fi
...
if [ ! -d "4_Annotation" ]; then
	/export/EC1680U/perl/bin/Trinity/5_Annotation_notTrinity.pl -i $INPUT -k $KEGG -t $TYPE -c $COMP 2>&1 | tee log/5_Annotation.log
fi
...
2_Trinity_interval_count_notTrinity.pl
3_Transcript_exp_quant_notTrinity.pl
4_Diff_expression.pl
5_Annotation_notTrinity.pl
6_Report.pl
...
% Not_Trinity_Input_Pipe.sh -i input.fofn -c comp.txt -k sce -t euk
-----
-i|--input:
