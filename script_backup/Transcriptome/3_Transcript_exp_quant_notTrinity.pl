...
my $docker_basal_cmd = "sudo docker run --rm -it -v $fastq_path:$fastq_path -v /export/EC1680U/kentchen:/export/EC1680U/kentchen -v `pwd`:`pwd` -w `pwd` kentchendocker/trinityrnaseq:latest";
...
if($quant_folder_count != $sample_count){
	my $trinity_quant_cmd = "align_and_estimate_abundance.pl ". 
	"--transcripts $trans_95 --seqType fq --samples_file $input --thread_count 32 ".
	"--est_method RSEM --aln_method bowtie2 --prep_reference ".
	"2> $log/align_and_estimate_abundance.log";
...
while(<$sample_in>){
	...
	my $sam_stats_cmd = "sam-stats $quant_sample_dir/bowtie2.bam > $quant_sample_dir/$sample.mappingStats.txt";
	...
	if(! -d "$quant_sample_dir"){
		...
		system("sudo chown -R $USER:users $quant_dir");
	}
...
#	my $sorted_bam_and_index = "picard SortSam I=$quant_sample_dir/bowtie2.bam.for_rsem.bam O=$quant_sample_dir/bowtie2.sorted.bam SO=coordinate TMP_DIR=./tmp CREATE_INDEX=true";
	my $sorted_bam_and_index = "samtools sort -@ 32 -o $quant_sample_dir/bowtie2.sorted.bam $quant_sample_dir/bowtie2.bam; samtools index $quant_sample_dir/bowtie2.sorted.bam";
#	my $sorted_bam_and_index = "picard SortSam I=$quant_sample_dir/bowtie2.bam.for_rsem.bam O=$quant_sample_dir/bowtie2.sorted.bam SO=coordinate TMP_DIR=./tmp CREATE_INDEX=true";
	if(! -e "$quant_sample_dir/$sample/bowtie2.sorted.bam"){system("$sorted_bam_and_index");}
#	if(! -e "$quant_sample_dir/$sample/bowtie2.sorted.bam.bai"){system("$sorted_bam_and_index");}
	...
	while(my $iso_txt = <$iso_file>){
		...
		$iso_txt =~ s/TPM/TPM ($sample)/g;
		my @iso_sp = split/\t/, $iso_txt;
		print $iso_report "$iso_sp[0]\t$iso_sp[2]\t$iso_sp[4]\t$iso_sp[5]\t$iso_sp[6]\n";
	}
	close $iso_file;
	close $iso_report;
...
while(<$all_RSEM_report>){
	...
	for(my $k=4; $k<=$#sp; $k+=$interval){
		print $final_RSEM_report "$sp[$k]\t";
	}
...
system("rm -rf $stat_dir/*bowtie* $stat_dir/*RSEM*");
