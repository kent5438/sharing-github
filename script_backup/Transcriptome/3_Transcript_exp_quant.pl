...
#my $docker_basal_cmd = "sudo docker run --rm -v /export/EC1680U/kentchen:/export/EC1680U/kentchen -v $fastq_path:$fastq_path -v `pwd`:`pwd` -w `pwd` kentchendocker/trinityrnaseq:latest";
my $trans_95 = "$stat_dir/Trinity.95.fasta";
my $quant_folder_count = `ls $quant_dir | grep -v report | grep -v multiqc | grep -v CMD_history | wc -l`; chomp($quant_folder_count);
...
#    my $sorted_bam_and_index = "picard SortSam I=$quant_sample_dir/bowtie2.bam O=$quant_sample_dir/bowtie2.sorted.bam SO=coordinate TMP_DIR=./tmp CREATE_INDEX=true";
	...
	# Generate FPKM+1 tmp file for log2FC calculation
...
system("sudo chown -R $USER:users $stat_dir");
